...
jQuery.butterfly.defaultOptions = {
  contentDefaultWidth: '640px', // For content (can be em, % or px) - null default means 50em if pxToEm is available or 700px otherwise (a good line length for legibility)
  contentDefaultHeight: '480px', // For content (can be em, % or px)
  mediaMaxWidth: '640px', // For images (can be em, % or px)
  mediaMaxHeight: '480px', // For images (can be em, % or px)
  treatAsMedia: true, // Set to true for content to be resized as if it's media (good for video content)
  ...
  closeButtonImage: '/profiles/vicuni/modules/custom/vu_picasa/img/close.png', // Set to the path of your close button image
  ...
  captionMinimizedHeight: 30,
  customCaptionHeight: 60
};
...
(function($, ResizeEvents) { // start closure
  ...
  $.fn.butterfly = function(options) {
    ...
    return this.each(function() {
      var tsmodal = $(this).data('tsmodal');
      var uid = $(this).data('uid');
      options['tsmodal'] = tsmodal;
      options['uid'] = uid;
      //options['description'] = false;
      //options['transcript'] = false;
      if (tsmodal && uid) {
        var description = $('#description-' + uid);
        var transcript = $('#transcript-' + uid);
        if (description) {
          description.hide();
          options['description'] = description.html();
        }
        if (transcript) {
          transcript.hide();
          options['transcript'] = transcript.html();
        }
  ...
  initLightBox = function(options) {
    ...
    options = (typeof options === 'undefined') ? $.butterfly.defaultOptions : $.extend({}, $.butterfly.defaultOptions, options);
  ...
  openLightBox = function(e) {
    ...
    options.href = href;
    options.title = title || (options.description && options.transcript ? options.description + '<br>' + options.transcript : undefined);
    ...
    setTimeout(function() {
      ...
      switch (options.linkType) {
        case 'image':
          ...
          $('#jb-window-inner').css('overflow', 'hidden');
          var caption = $('.jb-caption').first();
          var captionicon = $('.jb-caption').find('.caption-icon').first();

          // Hide caption icon initially.
          $(captionicon).hide();

          if (caption.length) {
            $(caption).height(options.captionMinimizedHeight);

            // Caption css overflow is set to hidden, hovering over it expands and shows the hidden text.
            // Display the rest of the caption when the user hover over it.
            $(caption).on('focus mouseenter touchstart', function() {
              var height = Math.min($(caption)[0].scrollHeight, $('#jb-window-inner').height());
              $(captionicon).hide();
            ...
            // Restore the caption area height to the default height.
            $(caption).on('blur mouseleave touchend', function() {
              $(caption).stop().animate({
                height: options.captionMinimizedHeight + 'px'
        ...
        case 'iframe':
          ...
          var caption = $('.jb-caption span').first();
          if (caption.length && options.captionMode == 'custom') {
            console.log('foooooo');
            var height = options.customCaptionHeight;
            caption.height(height);
            $('#jb-window-inner').height($('#jb-window-inner').height() + height);
          }
          break;
        case 'ajax':
      ...
      resizeLightBox.apply(this, [function() {
        ...
        if (options.captionMode == 'custom' && captionResized == false) {
          captionResized = true;
          //$('#jb-window-content').height($('#jb-window-inner').height()-options.customCaptionHeight);
          $('#jb-window-content > iframe').height($('#jb-window-content > iframe').height() - options.customCaptionHeight);
        }
        $('#jb-window-content').fadeIn(options.animationSpeed, function() {
          ...
          // Hides caption icon and disables expanding the caption area when the caption text is short and fits the default caption height.
          if (typeof window.getComputedStyle !== 'undefined') {
            var captionTextHeight = window.getComputedStyle($('.jb-caption .captionText')[0], null).getPropertyValue('height');
            var captionHeight = caption.height();
            if (parseFloat(captionTextHeight) <= captionHeight) {
              $(caption).off('focus mouseenter touchstart blur mouseleave touchend');
              captionicon.hide();
            }
            else {
              captionicon.fadeIn();
  ...
  resizeLightBox = function(callback) {
    ...
    if (options.linkType === 'image' || options.treatAsMedia) {
      ...
      if (options.captionMode = 'custom') {
        h = h + options.customCaptionHeight;
      }
  ...
  loadLightBoxContent = function(callback) {
    ...
    var options = $('#jb-overlay').data('options'),
      href = options.href,
      caption, imageIndex, gallerySet, prevControl, nextControl, prevLink, nextLink,
    ...
    var currentOfMultiple = gallerySet.length > 1 && gallerySet.isInSet('#' + options.linkID);
    if (currentOfMultiple) {
      imageIndex = '(' + (gallerySet.indexInSet('#' + options.linkID) + 1).toString() + '/' + gallerySet.length.toString() + ') ';
    }

    switch (options.captionMode) {
      case 'title':
        caption = options.title ? '<p class=\'jb-caption\' tabindex="0"><span class="caption-icon"></span><span class="captionText">' + imageIndex + options.title + '</span></p>' : '';
        break;
      case 'text':
        caption = options.linkText ? '<p class=\'jb-caption\'><span>' + imageIndex + options.linkText + '</span></p>' : '';
        break;
      case 'custom':
        caption = options.title ? '<p class=\'jb-caption\' tabindex="0"><span class="caption-icon"></span><span class="jb-video-caption captionText">' + imageIndex + options.title + '</span></p>' : '';
        break;
      default:
        caption = '';
        break;
    ...
    if (currentOfMultiple) {
      ...
      if (null != prevControl && 'object' == typeof prevControl && null != prevLink && 'object' == typeof prevLink && 'undefined' != typeof prevLink.attr('id')) {
      ...
      if (null != nextControl && 'object' == typeof nextControl && null != nextLink && 'object' == typeof nextLink && 'undefined' != typeof nextLink.attr('id')) {
    ...
    switch (options.linkType) {
      ...
      case 'image': // link to image
        ...
        $('#jb-window-content').empty().append(
          '<img src="' + href + '" alt="" style="max-width: 100%; max-height: 100%; float: left; display: block;" />' + caption
        );
      ...
      case 'iframe': // link to iframe URL
        ...
        var localCaption = $('.jb-caption');
        if (localCaption.length) {
          localCaption.remove();
        }
        $('#jb-window-content').empty().append(
          '<iframe src="' + href + '" width="100%" height="1000" style="display:block" />'
        );
        $('#jb-window-inner').append(caption);
  ...
  closeLightBox = function(evt) {
    ...
    var iFrame = $('#jb-window-content iframe');
    if (iFrame.length) {
      iFrame.remove();
    }
  ...
  $.fn.lightBoxContentHeight = function(relaxWidth) {
    ...
    var jbWindow = this.closest('#jb-window'),
      ...
      isImageType = jbWindow.hasClass('type-media') ? true : false,
      isImageMedia = jbWindow.hasClass('media-image') ? true : false,
      caption = $('.jb-caption').first().length,
      // always relax height
      currentWidth = jbWindowInner.width(),
      currentHeight = jbWindowInner.height(),
      currentLeft = jbWindowInner.css('left'),
    ...
    if (caption && options.captionMode != 'custom') {
      currentHeight += options.captionMinimizedHeight;
    }
    else if (caption && options.captionMode == 'custom') {
      currentHeight;
    }
    ...
    if (
      ...
      if (relaxWidth) {
        $('#jb-window-inner').width('auto').css({
          left: 0,
          top: 0
        });
      }
    ...
    if (caption && options.captionMode != 'custom') {
      fullHeight += options.captionMinimizedHeight;
    }
    else if (caption && options.captionMode == 'custom') {
      console.log('adding height')
      fullHeight += options.customCaptionHeight;
    }
    return fullHeight;
  };
  ...
  $.fn.indexInSet = function(currentElement) {
