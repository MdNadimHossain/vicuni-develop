(function($) {
  ...
  $.picasa = {
    albums: function(user, callback) {
      var url = '//picasaweb.google.com/data/feed/base/user/:user_id?alt=json&kind=album&hl=en_US&access=visible&fields=entry(id,link,media:group(media:content,media:description,media:keywords,media:title))&callback=?';
    ...
    images: function(user, album, callback) {
      var url = '//picasaweb.google.com/data/feed/base/user/:user_id/albumid/:album_id?alt=json&kind=photo&hl=en_US&fields=entry(title,gphoto:numphotos,media:group(media:content,media:thumbnail))&thumbsize=72c,288&imgmax=720&callback=?';
