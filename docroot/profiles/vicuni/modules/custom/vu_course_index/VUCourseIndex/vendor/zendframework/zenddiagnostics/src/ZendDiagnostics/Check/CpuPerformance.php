...
/**
 * Calculate CPU Performance by performing a Gauss-Legendre decimal expansion
 * of PI.
 *
 * The baseline has been calculated as the average time needed to calculate
 * 1000 digits of PI on an Amazon AWS EC2 Micro Instance.
 */
class CpuPerformance extends AbstractCheck implements CheckInterface {
  ...
  /**
   *
   * @param float $minPerformance The minimum performance ratio, where 1 is
   *   equal the computational performance of AWS EC2 Micro Instance. For
   *   example, a value of 2 means at least double the baseline experience,
   *   value of 0.5 means at least half the performance. Defaults to 0.5
   *
  ...
  /**
   * Run CPU benchmark and return a Success if the result is higher than
   * minimum performance, Failure if below and a warning if there was a problem
   * with calculating the value of PI.
   *
