...
class ObjectHydratorTest extends HydrationTestCase {
  ...
  /**
   * SELECT PARTIAL u.{id, status}, PARTIAL p.{phonenumber}, UPPER(u.name)
   * nameUpper FROM Doctrine\Tests\Models\CMS\CmsUser u JOIN u.phonenumbers p
   *
   * @dataProvider provideDataForUserEntityResult
   */
   ...
   *
   * DQL:
   * select c, b from Doctrine\Tests\Models\Forum\ForumCategory c inner join
   * c.boards b order by c.position asc, b.position asc
   *
   * Checks whether the boards are correctly assigned to the categories.
   *
   * The 'evil' result set that confuses the object population is displayed
   * below.
   *
   * c.id  | c.position | c.name   | boardPos | b.id | b.category_id (just for
   * clarity)
   *  1    | 0          | First    | 0        |   1  | 1
   *  2    | 0          | Second   | 0        |   2  | 2   <--
   *  1    | 0          | First    | 1        |   3  | 1
  ...
  /**
   * SELECT PARTIAL u.{id, status} AS user, PARTIAL a.{id, topic}, PARTIAL
   * c.{id, topic} FROM Doctrine\Tests\Models\CMS\CmsUser u LEFT JOIN
   * u.articles a LEFT JOIN a.comments c
   */
  ...
  /**
   * Checks if multiple joined multiple-valued collections is hydrated
   * correctly.
   *
   * SELECT PARTIAL u.{id, status}, PARTIAL g.{id, name}, PARTIAL
   * p.{phonenumber} FROM Doctrine\Tests\Models\CMS\CmsUser u
   *
   * @group DDC-809
   */
   ...
   * correctly.
   *
   * SELECT PARTIAL u.{id, status} As user, PARTIAL g.{id, name}, PARTIAL
   * p.{phonenumber} FROM Doctrine\Tests\Models\CMS\CmsUser u
   *
   * @group DDC-809
   */
  ...
  /**
   * SELECT PARTIAL u.{id, status}, PARTIAL p.{phonenumber}, UPPER(u.name) AS
   * nameUpper FROM Doctrine\Tests\Models\CMS\CmsUser u LEFT JOIN
   * u.phonenumbers u
   *
   * @group DDC-1358
   * @dataProvider provideDataForUserEntityResult
  ...
  /**
   * SELECT PARTIAL u.{id, status}, PARTIAL a.{id, city}, UPPER(u.name) AS
   * nameUpper FROM Doctrine\Tests\Models\CMS\CmsUser u JOIN u.address a
   *
   * @group DDC-1358
   * @dataProvider provideDataForUserEntityResult
   */
   ...
   * @expectedExceptionMessage The meta mapping for the discriminator column
   *   "c_discr" is missing for
   *   "Doctrine\Tests\Models\Company\CompanyFixContract" using the DQL alias
   *   "c".
   */
   ...
   *
   * @expectedException \Doctrine\ORM\Internal\Hydration\HydrationException
   * @expectedExceptionMessage The discriminator value "subworker" is invalid.
   *   It must be one of "person", "manager", "employee".
   */
