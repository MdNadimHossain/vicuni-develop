...
class Reader {
  ...
  /**
   * Gets the HTTP client object. If none is set, a new ZendHttp\Client will be
   * used.
   *
    ...
    if (static::$httpConditionalGet && $cache) {
      ...
      if ($response->getStatusCode() == 304) {
        ...
        if ($response->getHeaders()->get('ETag')) {
          $cache->setItem($cacheId . '_etag', $response->getHeaders()
            ->get('ETag')
            ->getFieldValue());
        }
        if ($response->getHeaders()->get('Last-Modified')) {
          $cache->setItem($cacheId . '_lastmodified', $response->getHeaders()
            ->get('Last-Modified')
            ->getFieldValue());
        }
      }
   ...
   *
   * @return self
   * @throws Exception\RuntimeException if response is not an
