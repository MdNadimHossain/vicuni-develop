...
class EntityRepositoryTest extends \Doctrine\Tests\OrmFunctionalTestCase {
  ...
  public function testFindByAssociationWithIntegerAsParameter() {
    ...
    $addresses = $repository->findBy(array(
      'user' => array(
        $user1->getId(),
        $user2->getId()
      )
  ...
  public function testFindByOrderByAssociation() {
    ...
    $this->assertEquals($resultAsc[0]->getEmail()
      ->getId(), $resultDesc[2]->getEmail()->getId());
    $this->assertEquals($resultAsc[2]->getEmail()
      ->getId(), $resultDesc[0]->getEmail()->getId());
  }
  ...
  public function testDefaultRepositoryClassName() {
    $this->assertEquals($this->_em->getConfiguration()
      ->getDefaultRepositoryClassName(), "Doctrine\ORM\EntityRepository");
    $this->_em->getConfiguration()
      ->setDefaultRepositoryClassName("Doctrine\Tests\Models\DDC753\DDC753DefaultRepository");
    $this->assertEquals($this->_em->getConfiguration()
      ->getDefaultRepositoryClassName(), "Doctrine\Tests\Models\DDC753\DDC753DefaultRepository");
    ...
    $this->assertEquals($this->_em->getConfiguration()
      ->getDefaultRepositoryClassName(), "Doctrine\Tests\Models\DDC753\DDC753DefaultRepository");
    $this->_em->getConfiguration()
      ->setDefaultRepositoryClassName("Doctrine\ORM\EntityRepository");
    $this->assertEquals($this->_em->getConfiguration()
      ->getDefaultRepositoryClassName(), "Doctrine\ORM\EntityRepository");
  }
   ...
   * @group DDC-753
   * @expectedException Doctrine\ORM\ORMException
   * @expectedExceptionMessage Invalid repository class
   *   'Doctrine\Tests\Models\DDC753\DDC753InvalidRepository'. It must be a
  ...
  public function testSetDefaultRepositoryInvalidClassError() {
    $this->assertEquals($this->_em->getConfiguration()
      ->getDefaultRepositoryClassName(), "Doctrine\ORM\EntityRepository");
    $this->_em->getConfiguration()
      ->setDefaultRepositoryClassName("Doctrine\Tests\Models\DDC753\DDC753InvalidRepository");
  }
   ...
   *   'Doctrine\Tests\Models\CMS\CmsUser#address', because it is the inverse
  ...
  public function testMatchingCriteriaContainsComparison() {
    ...
    $users = $repository->matching(new Criteria(Criteria::expr()
  ...
  public function testMatchingCriteriaNullAssocComparison() {
    ...
    $repository = $this->_em->getRepository('Doctrine\Tests\Models\CMS\CmsUser');
    $criteriaIsNull = Criteria::create()->where(Criteria::expr()
      ->isNull('email'));
    $criteriaEqNull = Criteria::create()->where(Criteria::expr()
      ->eq('email', NULL));
  ...
  public function testMatchingInjectionPrevented() {
    ...
    $result = $repository->matching(new Criteria(
      Criteria::expr()
        ->eq('username = ?; DELETE FROM cms_users; SELECT 1 WHERE 1', 'beberlei')
    ));
  ...
  public function testFindInjectionPrevented() {
    ...
    $repository->find(array(
      'username = ?; DELETE FROM cms_users; SELECT 1 WHERE 1' => 'test',
      'id' => 1
    ));
  }
  ...
  public function testFindByNullValueInInCondition() {
    ...
    $users = $this->_em->getRepository('Doctrine\Tests\Models\CMS\CmsUser')
      ->findBy(array('status' => array(NULL)));
