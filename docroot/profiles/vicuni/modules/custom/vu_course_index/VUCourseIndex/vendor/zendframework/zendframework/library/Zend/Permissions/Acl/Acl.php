...
class Acl implements AclInterface {
   ...
   * these - mixing string identifiers and objects is ok - to indicate the
   * Roles
   * from which the newly added Role will directly inherit.
   *
   * In order to resolve potential ambiguities with conflicting rules inherited
   * from different parents, the most recently added parent takes precedence
   * over parents that were previously added. In other words, the first parent
   * added will have the least priority, and the last parent added will have
   ...
   * OP_ADD specifics:
   *
   *      A rule is added that would allow one or more Roles access to [certain
   * $privileges upon] the specified Resource(s).
   *
   * OP_REMOVE specifics:
   *
   ...
   * does not apply would remain in the ACL.
   *
   ...
   * identifiers for, existing Resources/Roles, or they may be passed as arrays
   * of these - mixing string identifiers and objects is ok - to indicate the
   * Resources and Roles to which the rule applies. If either
   ...
   * applies only to certain privileges upon the Resource(s) in question. This
   * may be specified to be a single privilege with a string, and multiple
   * privileges may be specified as an array of strings.
   *
   * If $assert is provided, then its assert() method must return true in order
   * for the rule to apply. If $assert is provided with $roles, $resources, and
   * $privileges all equal to null, then a rule having a type of:
   *
   *      TYPE_ALLOW will imply a type of TYPE_DENY, and
   *
   *      TYPE_DENY will imply a type of TYPE_ALLOW
   *
   * when the rule's assertion fails. This is because the ACL needs to provide
   * expected behavior when an assertion upon the default ACL rule fails.
   *
   ...
   *
   * If either $role or $resource is null, then the query applies to all Roles
   * or all Resources, respectively. Both may be null to query whether the ACL
   * has a "blacklist" rule
   * (allow everything to all). By default, Zend\Permissions\Acl creates a
   * "whitelist" rule (deny everything to all), and this method would return
   * false unless this default has been overridden (i.e., by executing
   * $acl->allow()).
   *
   ...
   * Resource. In other words, this method returns true if and only if the Role
   * is allowed all privileges on the Resource.
   *
   * This method checks Role inheritance using a depth-first traversal of the
   * Role registry. The highest priority parent (i.e., the parent most recently
   * added) is checked first, and its respective parents are checked similarly
   * before the lower-priority parents of the Role are checked.
   *
   * @param  Role\RoleInterface|string $role
   * @param  Resource\ResourceInterface|string $resource
  ...
  /**
   * Performs a depth-first search of the Role DAG, starting at $role, in order
   * to find a rule allowing/denying $role access to all privileges upon
   * $resource
   *
   * This method returns true if a rule is found and allows access. If a rule
   * exists and denies access, then this method returns false. If no applicable
   * rule is found, then this method returns null.
   *
  ...
  /**
   * Visits an $role in order to look for a rule allowing/denying $role access
   * to all privileges upon $resource
   *
   * This method returns true if a rule is found and allows access. If a rule
   * exists and denies access, then this method returns false. If no applicable
   * rule is found, then this method returns null.
  ...
  /**
   * Performs a depth-first search of the Role DAG, starting at $role, in order
   * to find a rule allowing/denying $role access to a $privilege upon
   * $resource
   *
   * This method returns true if a rule is found and allows access. If a rule
   * exists and denies access, then this method returns false. If no applicable
