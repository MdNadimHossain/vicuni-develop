...
class Forward extends AbstractPlugin {
   ...
   *
   * @param  array $listeners Listener information; see getListenersToDetach()
   ...
   *
   * @param  string $name Controller name; either a class name or an alias used
   *   in the controller manager
   * @param  null|array $params Parameters with which to seed a custom
   *   RouteMatch object for the new controller
   *
   * @return mixed
   * @throws Exception\DomainException if composed controller does not define
   *   InjectApplicationEventInterface or Locator aware; or if the discovered
   *   controller is not dispatchable
   */
  public function dispatch($name, array $params = NULL) {
    ...
    if ($params !== NULL) {
      ...
      $routeMatch->setMatchedRouteName($event->getRouteMatch()
        ->getMatchedRouteName());
    ...
    $sharedEvents = $event->getApplication()
      ->getEventManager()
      ->getSharedManager();
  ...
  /**
   * Detach problem listeners specified by getListenersToDetach() and return an
   * array of information that will allow them to be reattached.
   *
  ...
  /**
   * Reattach all problem listeners detached by detachProblemListeners(), if
   * any.
   *
   * @param  SharedEvents $sharedEvents Shared event manager
