...
class ClassTableInheritanceTest extends \Doctrine\Tests\OrmFunctionalTestCase {
  ...
  public function testCRUD() {
    ...
    $guilherme = $this->_em->getRepository(get_class($employee))
      ->findOneBy(array('name' => 'Guilherme Blanco'));
  ...
  public function testQueryForInheritedSingleValuedAssociation() {
    ...
    $dqlManager = $this->_em->createQuery($dql)
  ...
  public function testExistsSubclass() {
    ...
    $this->assertFalse($this->_em->getUnitOfWork()
      ->getEntityPersister(get_class($manager))
      ->exists($manager));

    $this->_em->persist($manager);
    $this->_em->flush();

    $this->assertTrue($this->_em->getUnitOfWork()
      ->getEntityPersister(get_class($manager))
      ->exists($manager));
  }
