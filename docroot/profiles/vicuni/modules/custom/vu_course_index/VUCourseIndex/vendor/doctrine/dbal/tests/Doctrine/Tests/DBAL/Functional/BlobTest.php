...
class BlobTest extends \Doctrine\Tests\DbalFunctionalTestCase {
  public function setUp() {
    ...
    $this->_conn->exec($this->_conn->getDatabasePlatform()
      ->getTruncateTableSQL('blob_table'));
  }
  ...
  private function assertBinaryContains($text) {
    ...
    $blobValue = Type::getType('binary')
      ->convertToPHPValue($row['binaryfield'], $this->_conn->getDatabasePlatform());
  ...
  private function assertBlobContains($text) {
    ...
    $blobValue = Type::getType('blob')
      ->convertToPHPValue($row['blobfield'], $this->_conn->getDatabasePlatform());
