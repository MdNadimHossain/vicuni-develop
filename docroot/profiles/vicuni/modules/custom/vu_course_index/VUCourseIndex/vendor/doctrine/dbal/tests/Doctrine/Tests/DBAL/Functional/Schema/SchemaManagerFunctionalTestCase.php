...
class SchemaManagerFunctionalTestCase extends \Doctrine\Tests\DbalFunctionalTestCase {
  ...
  public function testDropAndCreateSequence() {
    if (!$this->_conn->getDatabasePlatform()->supportsSequences()) {
      $this->markTestSkipped($this->_conn->getDriver()
          ->getName() . ' does not support sequences.');
    }
  ...
  public function testListNamespaceNames() {
    ...
    if (!in_array('test_create_schema', $namespaces)) {
      $this->_conn->executeUpdate($this->_sm->getDatabasePlatform()
        ->getCreateSchemaSQL('test_create_schema'));
  ...
  public function createListTableColumns() {
    ...
    $table->addColumn('test', 'string', array(
      'length' => 255,
      'notnull' => FALSE,
      'default' => 'expected default'
    ));
    $table->addColumn('foo', 'text', array('notnull' => TRUE));
    $table->addColumn('bar', 'decimal', array(
      'precision' => 10,
      'scale' => 4,
      'notnull' => FALSE
    ));
  ...
  public function testListTableColumns() {
    ...
    $this->assertContains($columns['baz2']->gettype()->getName(), array(
      'time',
      'date',
      'datetime'
    ));
  ...
  public function testListTableColumnsWithFixedStringColumn() {
    ...
    $table->addColumn('column_char', 'string', array(
      'fixed' => TRUE,
      'length' => 2
    ));
  ...
  public function testAlterTableScenario() {
    ...
    $this->assertEquals(array('foo'), array_map('strtolower', $table->getIndex('foo_idx')
      ->getColumns()));
    ...
    $tableDiff = new \Doctrine\DBAL\Schema\TableDiff("alter_table");
    $tableDiff->fromTable = $table;
    $tableDiff->changedIndexes[] = new \Doctrine\DBAL\Schema\Index('foo_idx', array(
      'foo',
      'foreign_key_test'
    ));
    ...
    $tableDiff = new \Doctrine\DBAL\Schema\TableDiff("alter_table");
    $tableDiff->fromTable = $table;
    $tableDiff->renamedIndexes['foo_idx'] = new \Doctrine\DBAL\Schema\Index('bar_idx', array(
      'foo',
      'foreign_key_test'
    ));
    ...
    $tableDiff = new \Doctrine\DBAL\Schema\TableDiff("alter_table");
    $tableDiff->fromTable = $table;
    $tableDiff->removedIndexes[] = new \Doctrine\DBAL\Schema\Index('bar_idx', array(
      'foo',
      'foreign_key_test'
    ));
  ...
  public function testChangeColumnsTypeWithDefaultValue() {
    ...
    $tableDiff->changedColumns['col_string'] = new ColumnDiff(
      'col_string',
      new Column('col_string', Type::getType('string'), array(
        'default' => 'foo',
        'fixed' => TRUE
      )),
      array('fixed'),
  ...
  public function testListForeignKeysComposite() {
    ...
    $foreignKey = new \Doctrine\DBAL\Schema\ForeignKeyConstraint(
      array('id', 'foreign_key_test'), 'test_create_fk4', array(
      'id',
      'other_id'
  ...
  public function testListTableDetailsWithFullQualifiedTableName() {
    ...
    $defaultSchemaName = $this->_sm->getDatabasePlatform()
  ...
  public function testAlterColumnComment($comment1, $expectedComment1, $comment2, $expectedComment2) {
    ...
    $this->assertSame($expectedComment1, $onlineTable->getColumn('comment1')
      ->getComment());
    $this->assertSame($expectedComment2, $onlineTable->getColumn('comment2')
      ->getComment());
    ...
    $this->assertSame($expectedComment2, $onlineTable->getColumn('comment1')
      ->getComment());
    $this->assertSame($expectedComment1, $onlineTable->getColumn('comment2')
      ->getComment());
    $this->assertSame($expectedComment1, $onlineTable->getColumn('no_comment1')
      ->getComment());
    $this->assertSame($expectedComment2, $onlineTable->getColumn('no_comment2')
      ->getComment());
  }
