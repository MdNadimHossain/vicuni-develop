 ...
 * - ViewTemplateMapResolver (also aliased to
 * Zend\View\Resolver\TemplateMapResolver)
 * - ViewTemplatePathStack (also aliased to
 * Zend\View\Resolver\TemplatePathStack)
 * - ViewResolver (also aliased to Zend\View\Resolver\AggregateResolver and
 * ResolverInterface)
 * - ViewRenderer (also aliased to Zend\View\Renderer\PhpRenderer and
 * RendererInterface)
 * - ViewPhpRendererStrategy (also aliased to
 * Zend\View\Strategy\PhpRendererStrategy)
 * - View (also aliased to Zend\View\View)
 * - DefaultRenderingStrategy (also aliased to
 * Zend\Mvc\View\Http\DefaultRenderingStrategy)
 * - ExceptionStrategy (also aliased to Zend\Mvc\View\Http\ExceptionStrategy)
 * - RouteNotFoundStrategy (also aliased to
 * Zend\Mvc\View\Http\RouteNotFoundStrategy and 404Strategy)
 * - ViewModel
 */
class ViewManager extends AbstractListenerAggregate {
  ...
  public function attach(EventManagerInterface $events) {
    $this->listeners[] = $events->attach(MvcEvent::EVENT_BOOTSTRAP, array(
      $this,
      'onBootstrap'
    ), 10000);
  }
  ...
  public function onBootstrap($event) {
    ...
    $events->attach(MvcEvent::EVENT_DISPATCH_ERROR, array(
      $injectViewModelListener,
      'injectViewModel'
    ), -100);
    $events->attach(MvcEvent::EVENT_RENDER_ERROR, array(
      $injectViewModelListener,
      'injectViewModel'
    ), -100);
