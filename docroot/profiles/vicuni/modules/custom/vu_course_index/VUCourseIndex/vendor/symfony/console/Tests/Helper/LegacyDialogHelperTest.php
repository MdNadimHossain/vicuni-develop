...
class LegacyDialogHelperTest extends \PHPUnit_Framework_TestCase {
  public function testSelect() {
    ...
    $this->assertEquals(array(
      '0',
      '2'
    ), $dialog->select($this->getOutputStream(), 'What is your favorite superhero?', $heroes, NULL, FALSE, 'Input "%s" is not a superhero!', TRUE));
    $this->assertEquals(array(
      '0',
      '2'
    ), $dialog->select($this->getOutputStream(), 'What is your favorite superhero?', $heroes, NULL, FALSE, 'Input "%s" is not a superhero!', TRUE));
    $this->assertEquals(array(
      '0',
      '1'
    ), $dialog->select($this->getOutputStream(), 'What is your favorite superhero?', $heroes, '0,1', FALSE, 'Input "%s" is not a superhero!', TRUE));
    $this->assertEquals(array(
      '0',
      '1'
    ), $dialog->select($this->getOutputStream(), 'What is your favorite superhero?', $heroes, ' 0 , 1 ', FALSE, 'Input "%s" is not a superhero!', TRUE));
  ...
  public function testSelectOnErrorOutput() {
    ...
    $this->assertContains('Input "Stdout" is not a superhero!', stream_get_contents($output->getErrorOutput()
      ->getStream()));
  }
  ...
  public function testAskOnErrorOutput() {
    ...
    rewind($output->getErrorOutput()->getStream());
    $this->assertEquals('Where should output go?', stream_get_contents($output->getErrorOutput()
      ->getStream()));
  }
  ...
  public function testAskHiddenResponseOnErrorOutput() {
    ...
    rewind($output->getErrorOutput()->getStream());
    $this->assertContains('What time is it?', stream_get_contents($output->getErrorOutput()
      ->getStream()));
  }
  ...
  public function testAskAndValidate() {
    ...
    try {
      ...
      rewind($output->getErrorOutput()->getStream());
      $this->assertContains('What color was the white horse of Henry IV?', stream_get_contents($output->getErrorOutput()
        ->getStream()));
    }
  }
