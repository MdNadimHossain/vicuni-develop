...
class ClassMetadataTest extends \Doctrine\Tests\OrmTestCase {
  ...
  public function testSetDiscriminatorMapInGlobalNamespace() {
    ...
    $cm->initializeReflection(new RuntimeReflectionService());
    $cm->setDiscriminatorMap(array(
      'descr' => 'DoctrineGlobal_Article',
      'foo' => 'DoctrineGlobal_User'
    ));
  ...
  public function testUnderscoreNamingStrategyDefaults() {
    ...
    $this->assertEquals(array(
      'CMS_ADDRESS_ID',
      'CMS_USER_ID'
    ), $manyToManyMetadata->associationMappings['user']['joinTableColumns']);
   ...
   * @expectedExceptionMessage Query named "userById" in
   *   "Doctrine\Tests\Models\CMS\CmsUser" was already declared, but it must be
   *   declared only once
   */
   ...
   *
   * @expectedException \Doctrine\ORM\Mapping\MappingException
   * @expectedExceptionMessage Query named "find-all" in
   *   "Doctrine\Tests\Models\CMS\CmsUser" was already declared, but it must be
   *   declared only once
   */
   ...
   *
   * @expectedException \Doctrine\ORM\Mapping\MappingException
   * @expectedExceptionMessage Result set mapping named "find-all" in
   *   "Doctrine\Tests\Models\CMS\CmsUser" was already declared, but it must be
   *   declared only once
   */
   ...
   *
   * @expectedException \Doctrine\ORM\Mapping\MappingException
   * @expectedExceptionMessage Query name on entity class
   *   'Doctrine\Tests\Models\CMS\CmsUser' is not defined.
   */
   ...
   * @expectedException \Doctrine\ORM\Mapping\MappingException
   * @expectedExceptionMessage Result set mapping named "find-all" in
   *   "Doctrine\Tests\Models\CMS\CmsUser requires a entity class name.
   */
  ...
  /**
   * @expectedException \Doctrine\ORM\Mapping\MappingException
   * @expectedExceptionMessage Discriminator column name on entity class
   *   'Doctrine\Tests\Models\CMS\CmsUser' is not defined.
   */
   ...
   * @group DDC-964
   * @expectedException        Doctrine\ORM\Mapping\MappingException
   * @expectedExceptionMessage Invalid field override named
   *   'invalidPropertyName' for class
   ...
   *
   * @expectedException        Doctrine\ORM\Mapping\MappingException
   * @expectedExceptionMessage Entity Listener "\InvalidClassName" declared on
   *   "Doctrine\Tests\Models\CMS\CmsUser" not found.
   */
   ...
   *   "\Doctrine\Tests\Models\Company\CompanyContractListener" declared on
  ...
  public function testManyToManySelfReferencingNamingStrategyDefaults() {
    ...
    $this->assertEquals(array(
      'customtypeparent_source',
      'customtypeparent_target'
    ), $cm->associationMappings['friendsWithMe']['joinTableColumns']);
