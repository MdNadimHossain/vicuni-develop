...
class ArrayOfTypeSequence extends DefaultComplexType {
  ...
  protected function _getTypeBasedOnNestingLevel($singularType, $level) {
    ...
    return Wsdl::TYPES_NS . ':' . str_repeat('ArrayOf', $level) . ucfirst($this->getContext()
      ->translateType($singularType));
  }
   ...
   * @param  string $childType Qualified array items type (e.g. 'xsd:int',
   *   'tns:ArrayOfInt')
   * @param  string $phpArrayType PHP type (e.g. 'int[][]',
