...
class ToolbarListener implements ListenerAggregateInterface {
  ...
  public function onCollected(ProfilerEvent $event) {
    ...
    if ($headers->has('Content-Type')
      && FALSE === strpos($headers->get('Content-Type')
        ->getFieldValue(), 'html')
    ) {
      return;
    }
  ...
  /**
   * Tries to injects the toolbar into the view. The toolbar is only injected
   * in well formed HTML by replacing the closing body tag, leaving ESI
   * untouched.
   *
   * @param ProfilerEvent $event
   */
