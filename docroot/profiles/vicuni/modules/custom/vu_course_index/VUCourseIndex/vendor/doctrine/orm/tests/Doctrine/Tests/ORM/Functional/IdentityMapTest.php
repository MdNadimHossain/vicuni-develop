 ...
 * IdentityMapTest
 *
 * Tests correct behavior and usage of the identity map. Local values and
 * associations that are already fetched always prevail, unless explicitly
 * refreshed.
 *
...
class IdentityMapTest extends \Doctrine\Tests\OrmFunctionalTestCase {
  ...
  public function testSingleValuedAssociationIdentityMapBehaviorWithRefresh() {
    ...
    $this->_em->getConnection()
      ->executeUpdate('update cms_addresses set user_id = ?', array($user2->getId()));
  ...
  public function testCollectionValuedAssociationIdentityMapBehaviorWithRefreshQuery() {
    ...
    //external update to CmsAddress
    $this->_em->getConnection()
      ->executeUpdate('insert into cms_phonenumbers (phonenumber, user_id) VALUES (?,?)', array(
        999,
        $user->getId()
      ));
