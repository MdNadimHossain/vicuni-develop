...
class ApplicationTest extends \PHPUnit_Framework_TestCase {
  ...
  /**
   * Replaces the dynamic placeholders of the command help text with a static
   * version. The placeholder %command.full_name% includes the script path that
   * is not predictable and can not be tested against.
   */
  ...
  public function testAdd() {
    ...
    $commands = $application->all();
    $this->assertEquals(array($foo, $foo1), array(
      $commands['foo:bar'],
      $commands['foo:bar1']
   ...
   * @expectedExceptionMessage Command class "Foo5Command" is not correctly
   *   initialized. You probably forgot to call the parent constructor.
   */
  ...
  /**
   * @expectedException        Symfony\Component\Console\Exception\CommandNotFoundException
   * @expectedExceptionMessage There are no commands defined in the "bar"
   *   namespace.
   */
  ...
  public function testRenderException() {
    ...
    $tester->run(array('command' => 'foo'), array(
      'decorated' => FALSE,
  ...
  public function testCanCheckIfTerminalIsInteractive() {
    ...
    $this->assertFalse($tester->getInput()
      ->hasParameterOption(array('--no-interaction', '-n')));

    $inputStream = $application->getHelperSet()
      ->get('question')
      ->getInputStream();
