...
DQL;
  ...
  public function testLifecycleListener_ChangeUpdateChangeSet() {
    ...
    $this->_em->getEventManager()
      ->addEventListener(array('preUpdate'), $listener);
    ...
    $this->_em->getEventManager()
      ->removeEventListener(array('preUpdate'), $listener);
...
class LifecycleCallbackCascader {
  ...
  /**
   * @OneToMany(targetEntity="LifecycleCallbackTestEntity", mappedBy="cascader",
   *   cascade={"persist"})
   */
  public $entities;
