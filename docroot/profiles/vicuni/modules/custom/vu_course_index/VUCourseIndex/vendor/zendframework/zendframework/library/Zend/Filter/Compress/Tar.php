...
class Tar extends AbstractCompressionAlgorithm {
   ...
   *
   * @throws Exception\ExtensionNotLoadedException if Archive_Tar component not
   *   available
   */
  ...
  public function setArchive($archive) {
    $archive = str_replace(array(
      '/',
      '\\'
    ), DIRECTORY_SEPARATOR, (string) $archive);
   ...
   * @throws Exception\ExtensionNotLoadedException if bz2 mode selected but
   *   extension not loaded
   * @throws Exception\ExtensionNotLoadedException if gz mode selected but
   *   extension not loaded
   */
