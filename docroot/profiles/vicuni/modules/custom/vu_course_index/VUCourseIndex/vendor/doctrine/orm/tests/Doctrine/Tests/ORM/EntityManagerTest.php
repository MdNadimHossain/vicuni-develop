...
class EntityManagerTest extends \Doctrine\Tests\OrmTestCase {
  ...
  public function testCreateNamedNativeQuery() {
    ...
    $this->_em->getConfiguration()
      ->addNamedNativeQuery('foo', 'SELECT foo', $rsm);
  ...
  public function testCreateQueryBuilderAliasValid() {
    ...
    $this->assertEquals('SELECT u FROM Doctrine\Tests\Models\CMS\CmsUser u', $q->getQuery()
      ->getDql());
    $this->assertEquals('SELECT u FROM Doctrine\Tests\Models\CMS\CmsUser u', $q2->getQuery()
      ->getDql());

    $q3 = clone $q;

    $this->assertEquals('SELECT u FROM Doctrine\Tests\Models\CMS\CmsUser u', $q3->getQuery()
      ->getDql());
  }
  ...
  public function testTransactionalAcceptsVariousCallables() {
    $this->assertSame('callback', $this->_em->transactional(array(
      $this,
