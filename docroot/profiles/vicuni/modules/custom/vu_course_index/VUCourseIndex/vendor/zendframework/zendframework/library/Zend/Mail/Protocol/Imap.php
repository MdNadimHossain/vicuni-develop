...
class Imap {
   ...
   * Public constructor
   *
   * @param  string $host hostname or IP address of IMAP server, if given
   *   connect() is called
   * @param  int|null $port port of IMAP server, null for default (143 or 993
   *   for ssl)
  ...
  /**
   * read a response "line" (could also be more than one real line if response
   * has {..}<NL>) and do a simple decode
   *
   * @param  array|string $tokens decoded tokens are returned by reference, if
   *   $dontParse is true the unparsed line is returned here
   * @param  string $wantedTag check for this tag for response code. Default
   *   '*' is continuation tag.
  ...
  /**
   * read all lines of response until given tag is found (last line of
   * response)
   *
   ...
   *                                 the decoded tokens
   *
   * @return null|bool|array tokens if success, false if error, null if bad
   *   request
   */
   ...
   * @param  array $tokens additional parameters to command, use escapeString()
   *   to prepare
   ...
   * @param  bool $dontParse if true unparsed lines are returned instead of
   *   tokens
   *
...
//              $result[] = $this->escapeString($v);
   ...
   * is in this method
   *
   * @param  string $command can be 'EXAMINE' or 'SELECT' and this is used as
   *   command
   ...
   *
   * @param  string|array $items items to fetch from message(s) as string (if
   *   only one item) or array of strings
   * @param  int|array $from message for items or start message if $to !== null
   * @param  int|null $to if null only one message ($from) is fetched, else
   *   it's the last message, INF means last message available
   *
   * @throws Exception\RuntimeException
   * @return string|array if only one item of one message is fetched it's
   *   returned as string if items of one message are fetched it's returned as
   *   (name => value) if one items of messages are fetched it's returned as
   *   (msgno => value) if items of messages are fetched it's returned as
   *   (msgno => (name => value))
   */
   ...
   * get mailbox list
   *
   * this method can't be named after the IMAP command 'LIST', as list is a
   * reserved keyword
   *
   * @param  string $reference mailbox reference for list
   * @param  string $mailbox mailbox name match with wildcards
   *
   * @return array mailboxes that matched $mailbox as array(globalName =>
   *   array('delim' => .., 'flags' => ..))
   * @throws \Zend\Mail\Protocol\Exception\ExceptionInterface
   */
   ...
   * @param  int|null $to if null only one message ($from) is fetched, else
   *   it's the last message, INF means last message available
   * @param  string|null $mode '+' to add flags, '-' to remove flags,
   *   everything else sets the flags as given
   * @param  bool $silent if false the return values are the new flags for the
   *   wanted messages
   *
   * @return bool|array new flags if $silent is false, else true or false
   ...
   * @param $from
   * @param int|null $to if null only one message ($from) is fetched, else it's
   *   the last message, INF means last message available
   *
   * @return bool success
   */
   ...
   * do a search request
   *
   * This method is currently marked as internal as the API might change and is
   * not safe if you don't take precautions.
   *
   * @param array $params
   *
