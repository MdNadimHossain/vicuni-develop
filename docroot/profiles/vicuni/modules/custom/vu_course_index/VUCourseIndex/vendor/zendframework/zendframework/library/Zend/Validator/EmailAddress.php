...
class EmailAddress extends AbstractValidator {
   ...
   * Zend\Validator\Hostname::ALLOW_*
   ...
   *
   * @param  bool $mx Set allowed to true to validate for MX records, and false
   *   to not validate them
   *
   ...
   *
   * @param  bool $deep Set deep to true to perform a deep validation process
   *   for MX records
   *
   * @return EmailAddress Fluid Interface
   */
   ...
   * @param string $host
   *
   * @return bool Returns false when minimal one of the given addresses is not
   *   reserved
   */
  ...
  /**
   * Returns the found MX Record information after validation including weight
  ...
  protected function validateHostnamePart() {
    ...
    if (!$hostname) {
      ...
      foreach ($this->getHostnameValidator()
                 ->getMessages() as $code => $message) {
