...
/**
 * A Zend\Http\Cookies object is designed to contain and maintain HTTP cookies,
 * and should be used along with Zend\Http\Client in order to manage cookies
 * across HTTP requests and responses.
 *
 * The class contains an array of Zend\Http\Header\Cookie objects. Cookies can
 * be added automatically from a request or manually. Then, the Cookies class
 * can find and return the cookies needed for a specific HTTP request.
 *
 * A special parameter can be passed to all methods of this class that return
 * cookies: Cookies can be returned either in their native form (as
 * Zend\Http\Header\Cookie objects) or as strings - the later is suitable for
 * sending as the value of the "Cookie" header in an HTTP request. You can also
 * choose, when returning more than one cookie, whether to get an array of
 * strings
 * (by passing Zend\Http\Client\Cookies::COOKIE_STRING_ARRAY) or one unified
 * string for all cookies
 * (by passing Zend\Http\Client\Cookies::COOKIE_STRING_CONCAT).
 *
 * @link       http://wp.netscape.com/newsref/std/cookie_spec.html for some
 *   specs.
 */
class Cookies extends Headers {
  ...
  /**
   * Return all cookies as one long string (suitable for sending in an HTTP
   * request)
   *
   */
  ...
  /**
   * Add a cookie to the class. Cookie should be passed either as a
   * Zend\Http\Header\Cookie object or as a string - in which case an object is
   * created from the string.
   *
   * @param SetCookie|string $cookie
   * @param Uri\Uri|string $refUri Optional reference URI (for domain, path,
   *   secure)
   *
   ...
   *
   * @param int $retAs Whether to return cookies as objects of
   *   \Zend\Http\Header\SetCookie or as strings
   *
   * @return array|string
   */
  ...
  /**
   * Return an array of all cookies matching a specific request according to
   * the request URI, whether session cookies should be sent or not, and the
   * time to consider as "now" when checking cookie expiry time.
   *
   ...
   * @param int $retAs Whether to return cookies as objects of
   *   \Zend\Http\Header\Cookie or as strings
   ...
   *   \Zend\Http\Header\SetCookie or as strings
   *
   * @throws Exception\InvalidArgumentException if invalid URI specified or
   *   invalid $retAs value
  ...
  /**
   * Helper function to recursively flatten an array. Should be used when
   * exporting the cookies array (or parts of it)
   *
  ...
  /**
   * Return a subset of a domain-matching cookies that also match a specified
   * path
   *
   * @param array $domains
   * @param string $path
