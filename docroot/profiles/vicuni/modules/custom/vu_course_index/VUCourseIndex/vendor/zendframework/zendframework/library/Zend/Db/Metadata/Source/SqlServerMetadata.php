...
class SqlServerMetadata extends AbstractSource {
  ...
  protected function loadSchemaData() {
    ...
    $sql = 'SELECT ' . $p->quoteIdentifier('SCHEMA_NAME')
      . ' FROM ' . $p->quoteIdentifierChain(array(
        'INFORMATION_SCHEMA',
        'SCHEMATA'
      ))
  ...
  protected function loadTableNameData($schema) {
    ...
    $sql = 'SELECT ' . implode(', ', $isColumns)
      ...
      . ' LEFT JOIN ' . $p->quoteIdentifierChain(array(
        'INFORMATION_SCHEMA',
        'VIEWS'
      )) . ' v'
  ...
  protected function loadColumnData($table, $schema) {
    ...
    $sql = 'SELECT ' . implode(', ', $isColumns)
      ...
      . ' INNER JOIN ' . $p->quoteIdentifierChain(array(
        'INFORMATION_SCHEMA',
        'COLUMNS'
      )) . 'C'
      . ' ON ' . $p->quoteIdentifierChain(array('T', 'TABLE_SCHEMA'))
  ...
  protected function loadConstraintData($table, $schema) {
    ...
    $sql = 'SELECT ' . implode(', ', $isColumns)
      ...
      . ' ON ' . $p->quoteIdentifierChain(array(
        'RC',
        'UNIQUE_CONSTRAINT_SCHEMA'
      ))
      . '  = ' . $p->quoteIdentifierChain(array('KCU2', 'CONSTRAINT_SCHEMA'))
      . ' AND ' . $p->quoteIdentifierChain(array(
        'RC',
    ...
    $sql .= ' ORDER BY CASE ' . $p->quoteIdentifierChain(array(
        'TC',
        'CONSTRAINT_TYPE'
      ))
