...
abstract class AbstractPlatformTestCase extends \Doctrine\Tests\DbalTestCase {
  ...
  public function testGenerateTableWithMultiColumnUniqueIndex() {
    $table = new Table('test');
    $table->addColumn('foo', 'string', array(
      'notnull' => FALSE,
      'length' => 255
    ));
    $table->addColumn('bar', 'string', array(
      'notnull' => FALSE,
      'length' => 255
    ));
  ...
  public function testGeneratesIndexCreationSql() {
    $indexDef = new \Doctrine\DBAL\Schema\Index('my_idx', array(
      'user_name',
      'last_login'
    ));
  ...
  public function testGeneratesUniqueIndexCreationSql() {
    $indexDef = new \Doctrine\DBAL\Schema\Index('index_name', array(
      'test',
      'test2'
    ), TRUE);
  ...
  public function testGeneratesPartialIndexesSqlOnlyWhenSupportingPartialIndexes() {
    ...
    $indexDef = new \Doctrine\DBAL\Schema\Index('name', array(
      'test',
      'test2'
    ), FALSE, FALSE, array(), array('where' => $where));
    $uniqueIndex = new \Doctrine\DBAL\Schema\Index('name', array(
      'test',
      'test2'
    ), TRUE, FALSE, array(), array('where' => $where));
  ...
  public function testGetCreateTableSqlDispatchEvent() {
    $listenerMock = $this->getMock('GetCreateTableSqlDispatchEvenListener', array(
      'onSchemaCreateTable',
  ...
  public function testQuotedColumnInForeignKeyPropagation() {
    ...
    $table->addForeignKeyConstraint($foreignTable, array(
      'create',
      'foo',
      '`bar`'
    ), array(
      ...
      'bar',
      '`foo-bar`'
    ), array(), 'FK_WITH_RESERVED_KEYWORD');
    ...
    $table->addForeignKeyConstraint($foreignTable, array(
      ...
      'bar',
      '`foo-bar`'
    ), array(), 'FK_WITH_NON_RESERVED_KEYWORD');
    ...
    $table->addForeignKeyConstraint($foreignTable, array(
      ...
      'bar',
      '`foo-bar`'
    ), array(), 'FK_WITH_INTENDED_QUOTATION');
  ...
  public function testQuotesAlterTableChangeColumnLength() {
    ...
    $fromTable->addColumn('unquoted1', 'string', array(
      'comment' => 'Unquoted 1',
      'length' => 10
    ));
    $fromTable->addColumn('unquoted2', 'string', array(
      'comment' => 'Unquoted 2',
      'length' => 10
    ));
    $fromTable->addColumn('unquoted3', 'string', array(
      'comment' => 'Unquoted 3',
      'length' => 10
    ));

    $fromTable->addColumn('create', 'string', array(
      'comment' => 'Reserved keyword 1',
      'length' => 10
    ));
    $fromTable->addColumn('table', 'string', array(
      'comment' => 'Reserved keyword 2',
      'length' => 10
    ));
    $fromTable->addColumn('select', 'string', array(
      'comment' => 'Reserved keyword 3',
      'length' => 10
    ));

    $toTable = new Table('mytable');

    $toTable->addColumn('unquoted1', 'string', array(
      'comment' => 'Unquoted 1',
      'length' => 255
    ));
    $toTable->addColumn('unquoted2', 'string', array(
      'comment' => 'Unquoted 2',
      'length' => 255
    ));
    $toTable->addColumn('unquoted3', 'string', array(
      'comment' => 'Unquoted 3',
      'length' => 255
    ));

    $toTable->addColumn('create', 'string', array(
      'comment' => 'Reserved keyword 1',
      'length' => 255
    ));
    $toTable->addColumn('table', 'string', array(
      'comment' => 'Reserved keyword 2',
      'length' => 255
    ));
    $toTable->addColumn('select', 'string', array(
      'comment' => 'Reserved keyword 3',
      'length' => 255
    ));
  ...
  public function testGetCommentOnColumnSQL() {
    $this->assertSame(
      ...
      array(
        $this->_platform->getCommentOnColumnSQL('foo', 'bar', 'comment'),
        // regular identifiers
        $this->_platform->getCommentOnColumnSQL('`Foo`', '`BAR`', 'comment'),
        // explicitly quoted identifiers
        $this->_platform->getCommentOnColumnSQL('select', 'from', 'comment'),
  ...
  public function testAlterStringToFixedString() {
    ...
    $tableDiff->changedColumns['name'] = new \Doctrine\DBAL\Schema\ColumnDiff(
      ...
      'name', \Doctrine\DBAL\Types\Type::getType('string'), array(
        'fixed' => TRUE,
        'length' => 2
      )
    ),
