...
class TypeConversionTest extends \Doctrine\Tests\DbalFunctionalTestCase {
  ...
  public function setUp() {
    ...
    $table->addColumn('test_decimal', 'decimal', array(
      'notnull' => FALSE,
      'scale' => 2,
      'precision' => 10
    ));
  ...
  public function testIdempotentDataConversion($type, $originalValue, $expectedPhpType) {
    ...
    $this->_conn->insert('type_conversion', array(
      'id' => ++self::$typeCounter,
      $columnName => $insertionValue
    ));
    ...
    if ($type !== "datetimetz") {
      ...
      if ($originalValue instanceof \DateTime) {
        $this->assertEquals($originalValue->getTimezone()
          ->getName(), $actualDbValue->getTimezone()
          ->getName(), "Timezones should be the same.");
      }
    }
  }
}
