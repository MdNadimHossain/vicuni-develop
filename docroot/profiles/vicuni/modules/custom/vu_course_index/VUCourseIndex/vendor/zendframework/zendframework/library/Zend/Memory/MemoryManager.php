...
class MemoryManager {
   ...
   *
   * It also represents objects access history. Last accessed objects are moved
   * to the end of array
   *
   * array(
   ...
   *
   * It's used to reduce number of calls necessary to trace objects'
   * modifications Modification is not processed by memory manager until we do
   * not switch to another object. So we have to trace only _first_ object
   * modification and do nothing for others
   *
  ...
  public function __destruct() {
    if ($this->cache !== NULL) {
      if ($this->cache instanceof ClearByNamespaceCacheStorage) {
        $this->cache->clearByNamespace($this->cache->getOptions()
          ->getNamespace());
      }
