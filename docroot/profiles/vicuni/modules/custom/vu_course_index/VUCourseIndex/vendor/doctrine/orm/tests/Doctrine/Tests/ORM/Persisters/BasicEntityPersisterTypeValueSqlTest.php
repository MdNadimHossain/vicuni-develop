...
class BasicEntityPersisterTypeValueSqlTest extends \Doctrine\Tests\OrmTestCase {
  ...
  public function testUpdateUsesTypeValuesSQL() {
    ...
    $this->_em->getUnitOfWork()
      ->registerManaged($parent, array('id' => 1), array(
        'customInteger' => 0,
        'child' => NULL
      ));
    $this->_em->getUnitOfWork()
      ->registerManaged($child, array('id' => 1), array());

    $this->_em->getUnitOfWork()
      ->propertyChanged($parent, 'customInteger', 0, 1);
    $this->_em->getUnitOfWork()
      ->propertyChanged($parent, 'child', NULL, $child);
  ...
  public function testGetSelectConditionSQLUsesTypeValuesSQL() {
    ...
    $sql = $method->invoke($this->_persister, array(
      'customInteger' => 1,
      'child' => 1
    ));
