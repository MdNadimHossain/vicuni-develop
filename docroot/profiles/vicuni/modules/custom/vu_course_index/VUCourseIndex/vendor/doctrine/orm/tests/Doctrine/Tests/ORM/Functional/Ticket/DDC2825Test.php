...
/**
 * This class makes tests on the correct use of a database schema when entities
 * are stored
 *
 * @group DDC-2825
 */
class DDC2825Test extends \Doctrine\Tests\OrmFunctionalTestCase {
  ...
  public function testClassSchemaMappingsValidity($className, $expectedSchemaName, $expectedTableName) {
    ...
    $platform = $this->_em->getConnection()->getDatabasePlatform();
    $quotedTableName = $this->_em->getConfiguration()
      ->getQuoteStrategy()
      ->getTableName($classMetadata, $platform);
  ...
  public function getTestedClasses() {
    return array(
      array(
        ExplicitSchemaAndTable::CLASSNAME,
        'explicit_schema',
        'explicit_table'
      ),
      array(
        SchemaAndTableInTableName::CLASSNAME,
        'implicit_schema',
        'implicit_table'
      ),
      array(
        DDC2825ClassWithImplicitlyDefinedSchemaAndQuotedTableName::CLASSNAME,
        'myschema',
        'order'
      ),
    );
