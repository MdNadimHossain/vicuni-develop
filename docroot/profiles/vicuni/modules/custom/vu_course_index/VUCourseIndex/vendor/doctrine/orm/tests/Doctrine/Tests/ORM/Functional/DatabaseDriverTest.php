...
class DatabaseDriverTest extends DatabaseDriverTestCase {
  ...
  public function testLoadMetadataWithForeignKeyFromDatabase() {
    ...
    $metadatas = $this->extractClassMetadata(array(
      "DbdriverBar",
      "DbdriverBaz"
    ));
  ...
  public function testIgnoreManyToManyTableWithoutFurtherForeignKeyDetails() {
    ...
    $metadatas = $this->convertToClassMetadata(array(
      $tableA,
      $tableB
    ), array($tableMany));
  ...
  public function testLoadMetadataFromDatabaseDetail() {
    ...
    $table->addColumn('column_decimal', 'decimal', array(
      'precision' => 4,
      'scale' => 3
    ));
    ...
    $table->addUniqueIndex(array(
      'column_unique_index1',
      'column_unique_index2'
    ), 'unique_index1');
    ...
    if (!$this->_em->getConnection()
          ->getDatabasePlatform() instanceof PostgreSqlPlatform AND
      !$this->_em->getConnection()
          ->getDatabasePlatform() instanceof SQLServerPlatform
    ) {
