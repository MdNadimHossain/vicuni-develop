...
class Ldap extends AbstractAdapter {
  ...
  /**
   * The DN of the authenticated account. Used to retrieve the account entry on
   * request.
   *
   * @var string
   */
   ...
   *
   * @return ZendLdap\Ldap The Zend\Ldap\Ldap object used to authenticate the
   *   credentials
   */
   ...
   *
   * This resembles the feature {@see
   * Zend\Authentication\Adapter\DbTable::getResultRowObject()}. Closes ZF-6813
   *
  ...
  public function getAccountObject(array $returnAttribs = array(), array $omitAttribs = array()) {
    ...
    $entry = $this->getLdap()
      ->getEntry($this->authenticatedDn, $returnAttribs, TRUE);
