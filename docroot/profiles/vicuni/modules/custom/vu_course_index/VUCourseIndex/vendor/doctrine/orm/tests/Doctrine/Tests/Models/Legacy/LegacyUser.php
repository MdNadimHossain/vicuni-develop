...
class LegacyUser {
  ...
  public $_articles;
  /**
   * @OneToMany(targetEntity="LegacyUserReference", mappedBy="_source",
   *   cascade={"remove"})
   */
  public $_references;
  /**
   * @ManyToMany(targetEntity="LegacyCar", inversedBy="_users",
   *   cascade={"persist", "merge"})
   * @JoinTable(name="legacy_users_cars",
   *      joinColumns={@JoinColumn(name="iUserId",
   *   referencedColumnName="iUserId")},
   *   inverseJoinColumns={@JoinColumn(name="iCarId",
   *   referencedColumnName="iCarId")}
   *      )
   */
  public $_cars;
