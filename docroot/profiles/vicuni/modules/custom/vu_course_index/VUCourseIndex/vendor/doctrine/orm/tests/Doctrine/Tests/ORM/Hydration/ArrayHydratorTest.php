...
class ArrayHydratorTest extends HydrationTestCase {
  ...
  /**
   * select u.id, u.status, p.phonenumber, upper(u.name) nameUpper, a.id,
   * a.topic from User u join u.phonenumbers p join u.articles a
   * =
   * select u.id, u.status, p.phonenumber, upper(u.name) AS u___0, a.id,
   * a.topic
   * from USERS u
  ...
  /**
   * select u.id, u.status, p.phonenumber, upper(u.name) nameUpper, a.id,
   * a.topic, c.id, c.topic from User u join u.phonenumbers p join u.articles a
   * left join a.comments c
   * =
   * select u.id, u.status, p.phonenumber, upper(u.name) AS u___0, a.id,
   * a.topic, c.id, c.topic from USERS u inner join PHONENUMBERS p ON u.id =
   * p.user_id inner join ARTICLES a ON u.id = a.user_id left outer join
  ...
  public function testMissingIdForRootEntity($userEntityKey) {
    ...
    $this->assertEquals(array(
      'id' => 1,
      'status' => 'developer'
    ), $result[0][$userEntityKey]);
    $this->assertNull($result[1][$userEntityKey]);
    $this->assertEquals(array(
      'id' => 2,
      'status' => 'developer'
    ), $result[2][$userEntityKey]);
