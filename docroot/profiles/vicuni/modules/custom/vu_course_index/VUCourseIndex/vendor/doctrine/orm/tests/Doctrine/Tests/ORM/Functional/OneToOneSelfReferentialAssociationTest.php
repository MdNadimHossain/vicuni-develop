 ...
 * inheritance). Relation is defined as the mentor that a customer choose. The
 * mentor could help only one other customer, while a customer can choose only
 * one mentor for receiving support. Inverse side is not present.
 */
class OneToOneSelfReferentialAssociationTest extends \Doctrine\Tests\OrmFunctionalTestCase {
  ...
  public function assertForeignKeyIs($value) {
    $foreignKey = $this->_em->getConnection()
      ->executeQuery('SELECT mentor_id FROM ecommerce_customers WHERE id=?', array($this->customer->getId()))
      ->fetchColumn();
