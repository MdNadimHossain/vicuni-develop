...
class FilterChain extends AbstractFilter implements Countable {
   ...
   *
   * @param  callable|FilterInterface $callback A Filter implementation or
   *   valid PHP callback
   * @param  int $priority Priority at which to enqueue filter; defaults to
   *   1000 (higher executes earlier)
   *
   * @throws Exception\InvalidArgumentException
   * @return self
   */
   ...
   *
   * Retrieves the filter from the attached plugin manager, and then calls
   * attach() with the retrieved instance.
   *
   * @param  string $name
   * @param  mixed $options
   * @param  int $priority Priority at which to enqueue filter; defaults to
