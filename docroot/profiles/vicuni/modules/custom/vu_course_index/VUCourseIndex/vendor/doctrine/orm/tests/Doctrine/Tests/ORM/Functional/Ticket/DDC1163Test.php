...
class DDC1163Test extends \Doctrine\Tests\OrmFunctionalTestCase {
  ...
  /**
   * We want Doctrine to instantiate a lazy-load proxy for the previously
   * created
   * 'SpecialProduct' and register it.
   *
   * When Doctrine loads the 'ProxyHolder', it will do just that because the
   * 'ProxyHolder' references the 'SpecialProduct'.
   */
  ...
  private function setPropertyAndAssignTagToSpecialProduct() {
    ...
    $this->assertEquals(
      ...
      $this->_em->getUnitOfWork()
        ->getEntityPersister(get_class($specialProduct))
