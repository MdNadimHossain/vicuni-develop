...
class DDC2790Test extends \Doctrine\Tests\OrmFunctionalTestCase {
  ...
  /**
   * Verifies that entities scheduled for deletion are not treated as updated
   * by UoW, even if their properties are changed after the remove() call
   */
  public function testIssue() {
    $this->_em->getEventManager()
      ->addEventListener(Events::onFlush, new OnFlushListener);
...
class OnFlushListener {
  /**
   * onFLush listener that tries to cancel deletions by calling persist if the
   * entity is listed as updated in UoW
   */
