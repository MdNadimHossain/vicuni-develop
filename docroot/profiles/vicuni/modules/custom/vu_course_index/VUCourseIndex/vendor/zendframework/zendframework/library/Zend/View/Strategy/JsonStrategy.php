...
class JsonStrategy extends AbstractListenerAggregate {
  ...
  /**
   * Multibyte character sets that will trigger a binary
  ...
  public function attach(EventManagerInterface $events, $priority = 1) {
    $this->listeners[] = $events->attach(ViewEvent::EVENT_RENDERER, array(
      $this,
      'selectRenderer'
    ), $priority);
    $this->listeners[] = $events->attach(ViewEvent::EVENT_RESPONSE, array(
      $this,
      'injectResponse'
    ), $priority);
  }
  ...
  /**
   * Inject the response with the JSON payload and appropriate Content-Type
   * header
   *
   * @param  ViewEvent $e
   *
