...
class ManyToManyBasicAssociationTest extends \Doctrine\Tests\OrmFunctionalTestCase {
  ...
  public function testBasicManyToManyJoin() {
    ...
    $this->assertEquals(\Doctrine\ORM\UnitOfWork::STATE_MANAGED, $this->_em->getUnitOfWork()
      ->getEntityState($result[0]));
    $this->assertEquals(\Doctrine\ORM\UnitOfWork::STATE_MANAGED, $this->_em->getUnitOfWork()
      ->getEntityState($groups[0]));
  ...
  public function testRetrieveManyToManyAndAddMore() {
    ...
    $this->assertEquals(3, count($freshUser->getGroups()
      ->getSnapshot()), "Snapshot of CmsUser::groups should contain 3 entries.");
  ...
  public function testUpdateDeleteSizeSubselectQueries() {
    $this->_em->createQuery("DELETE Doctrine\Tests\Models\CMS\CmsUser u WHERE SIZE(u.groups) = 10")
      ->execute();
    $this->_em->createQuery("UPDATE Doctrine\Tests\Models\CMS\CmsUser u SET u.status = 'inactive' WHERE SIZE(u.groups) = 10")
      ->execute();
  }
