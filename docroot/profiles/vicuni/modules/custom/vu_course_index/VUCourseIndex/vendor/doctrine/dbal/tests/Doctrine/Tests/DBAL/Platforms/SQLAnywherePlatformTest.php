...
class SQLAnywherePlatformTest extends AbstractPlatformTestCase {
  ...
  public function testGeneratesCreateTableSQLWithForeignKeyConstraints() {
    ...
    $table->setPrimaryKey(array('id'));
    $table->addForeignKeyConstraint('foreign_table', array(
      'fk_1',
      'fk_2'
    ), array('pk_1', 'pk_2'));
  ...
  public function testGeneratesUniqueConstraintDeclarationSQL() {
    ...
    $this->assertEquals(
      'UNIQUE (a, b)',
      $this->_platform->getUniqueConstraintDeclarationSQL(NULL, new Index(NULL, array(
        'a',
        'b'
      ), TRUE, FALSE))
    );
  ...
  public function testGeneratesForeignKeyConstraintsWithAdvancedPlatformOptionsSQL() {
    $this->assertEquals(
      ...
      $this->_platform->getForeignKeyDeclarationSQL(
        new ForeignKeyConstraint(array('a', 'b'), 'foreign_table', array(
          'c',
          'd'
        ), 'fk', array(
          'notnull' => TRUE,
  ...
  public function testCannotGenerateForeignKeyConstraintSQLWithEmptyLocalColumns() {
    $this->setExpectedException('\InvalidArgumentException');
    $this->_platform->getForeignKeyDeclarationSQL(new ForeignKeyConstraint(array(), 'foreign_tbl', array(
      'c',
      'd'
    )));
  }
  ...
  public function testCannotGenerateForeignKeyConstraintSQLWithEmptyForeignColumns() {
    $this->setExpectedException('\InvalidArgumentException');
    $this->_platform->getForeignKeyDeclarationSQL(new ForeignKeyConstraint(array(
      'a',
      'b'
    ), 'foreign_tbl', array()));
  }
  ...
  public function testReturnsBinaryTypeDeclarationSQL() {
    ...
    $this->assertSame('BINARY(1)', $this->_platform->getBinaryTypeDeclarationSQL(array(
      'fixed' => TRUE,
      'length' => 0
    )));
    $this->assertSame('BINARY(32767)', $this->_platform->getBinaryTypeDeclarationSQL(array(
      'fixed' => TRUE,
      'length' => 32767
    )));
    $this->assertSame('LONG BINARY', $this->_platform->getBinaryTypeDeclarationSQL(array(
      'fixed' => TRUE,
      'length' => 32768
    )));
  }
