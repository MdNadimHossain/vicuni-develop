...
class Json {
  ...
  /**
   * To check the allowed nesting depth of the XML tree during xml2json
   * conversion.
   *
   * @var int
   */
   ...
   *
   * NOTE: Encoding native javascript expressions are possible using
   * Zend\Json\Expr. You can enable this by setting
   * $options['enableJsonExprFinder'] = true
   *
   * @see Zend\Json\Expr
   *
   * @param  mixed $valueToEncode
   * @param  bool $cycleCheck Optional; whether or not to check for object
   ...
   * the XML elements are stored in the PHP array, it is returned to the
   * caller.
   *
   * @param SimpleXMLElement $simpleXmlElementObject
   ...
   * @throws \Zend\Json\Exception\RuntimeException if the input not a XML
   *   formatted string
   */
