...
class Headers implements Countable, Iterator {
   ...
   * of HeaderInterface object will be delayed until they are retrieved by
   * either get() or current()
   *
   * @throws Exception\InvalidArgumentException
   *
  ...
  /**
   * Add a Header\Interface to this container, for raw values see {@link
   * addHeaderLine()} and {@link addHeaders()}
   *
   ...
   *
   * @param  string|Header\HeaderInterface field name or specific header
   *   instance to remove
   *
   * @return bool
   */
   ...
   * @param  string $name
   *
   * @return bool|ArrayIterator|Header\HeaderInterface Returns false if there
   *   is no headers with $name in this contain, an ArrayIterator if the header
   *   is a MultipleHeadersInterface instance and finally returns
  ...
  /**
   * Return the number of headers in this contain, if all headers have not been
  ...
  protected function lazyLoadHeader($index) {
    ...
    $class = ($this->getPluginClassLoader()
      ->load($key)) ?: 'Zend\Mail\Header\GenericHeader';
