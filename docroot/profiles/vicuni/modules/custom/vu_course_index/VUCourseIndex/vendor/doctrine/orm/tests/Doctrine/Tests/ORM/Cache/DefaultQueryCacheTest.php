...
class DefaultQueryCacheTest extends OrmTestCase {
  ...
  public function testPutBasicQueryResult() {
    ...
    for ($i = 0; $i < 4; $i++) {
      ...
      $this->em->getUnitOfWork()
        ->registerManaged($entity, array('id' => $i), array('name' => $name));
    }
  ...
  public function testPutToOneAssociationQueryResult() {
    ...
    $rsm->addJoinedEntityFromClassMetadata(State::CLASSNAME, 's', 'c', 'state', array(
      'id' => 'state_id',
      'name' => 'state_name'
    ));
    ...
    for ($i = 0; $i < 4; $i++) {
      ...
      $uow->registerManaged($city, array('id' => $city->getId()), array(
        'name' => $city->getName(),
        'state' => $state
      ));
    }
  ...
  public function testPutToManyAssociationQueryResult() {
    ...
    for ($i = 0; $i < 4; $i++) {
      ...
      $uow->registerManaged($city1, array('id' => $city1->getId()), array(
        'name' => $city1->getName(),
        'state' => $state
      ));
      $uow->registerManaged($city2, array('id' => $city2->getId()), array(
        'name' => $city2->getName(),
        'state' => $state
      ));
      $uow->registerManaged($state, array('id' => $state->getId()), array(
        'name' => $state->getName(),
   ...
   * @expectedExceptionMessage Entity association field
   *   "Doctrine\Tests\Models\Cache\City#travels" not configured as part of the
   *   second-level cache.
   */
  ...
  /**
   * @expectedException Doctrine\ORM\Cache\CacheException
   * @expectedExceptionMessage Second level cache does not support scalar
   *   results.
   */
  ...
  /**
   * @expectedException Doctrine\ORM\Cache\CacheException
   * @expectedExceptionMessage Second level cache does not support multiple
   *   root entities.
   */
  ...
  /**
   * @expectedException Doctrine\ORM\Cache\CacheException
   * @expectedExceptionMessage Entity
   *   "Doctrine\Tests\Models\Generic\BooleanModel" not configured as part of
  ...
  public function testNotCacheableEntityException() {
    ...
    for ($i = 0; $i < 4; $i++) {
      ...
      $this->em->getUnitOfWork()
        ->registerManaged($entity, array('id' => $i), array('booleanField' => $boolean));
    }
