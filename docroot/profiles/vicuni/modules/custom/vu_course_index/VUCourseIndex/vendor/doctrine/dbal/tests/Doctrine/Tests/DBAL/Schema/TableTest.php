...
class TableTest extends \Doctrine\Tests\DbalTestCase {
  ...
  public function testAddForeignKeyAddsImplicitIndexIfIndexColumnsDoNotSpan() {
    ...
    $this->assertSame(array('baz', 'bar'), $table->getIndex('composite_idx')
      ->getColumns());
    $this->assertSame(array('bar', 'baz', 'bloo'), $table->getIndex('full_idx')
      ->getColumns());
    $this->assertSame(array(
      'bar',
      'baz'
    ), $table->getIndex('idx_8c73652176ff8caa78240498')->getColumns());
  }
  ...
  public function testAllowsAddingDuplicateIndexesBasedOnColumns() {
    ...
    $this->assertSame(array('bar'), $table->getIndex('bar_idx')->getColumns());
    $this->assertSame(array('bar'), $table->getIndex('duplicate_idx')
      ->getColumns());
  }
  ...
  public function testAllowsAddingFulfillingIndexesBasedOnColumns() {
    ...
    $this->assertSame(array('bar'), $table->getIndex('bar_idx')->getColumns());
    $this->assertSame(array('bar', 'baz'), $table->getIndex('fulfilling_idx')
      ->getColumns());
  }
  ...
  public function testAddForeignKeyWithQuotedColumnsAndTable() {
    ...
    $table->addColumn('"foo"', 'integer');
    $table->addColumn('bar', 'integer');
    $table->addForeignKeyConstraint('"boing"', array(
      '"foo"',
      '"bar"'
    ), array("id"));
  }
  ...
  public function testRenameIndex() {
    ...
    $this->assertEquals(new Index('uniq_new', array(
      'bar',
      'baz'
    ), TRUE), $table->getIndex('uniq_new'));
