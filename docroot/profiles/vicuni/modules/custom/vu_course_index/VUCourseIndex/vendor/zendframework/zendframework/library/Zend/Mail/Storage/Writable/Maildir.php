...
class Maildir extends Folder\Maildir implements WritableInterface {
   ...
   *   is set
   * @param   string|\Zend\Mail\Storage\Folder $parentFolder parent folder for
   *   new folder, else root folder is parent
   *
   * @throws \Zend\Mail\Storage\Exception\RuntimeException
   ...
   *
   * @param  string|\Zend\Mail\Storage\Folder $oldName name or instance of
   *   folder
   ...
   *
   * This is nearly the format defined in the maildir standard. The microtime()
   * call should already create a uniqueid, the pid is for multicore/-cpu
   * machine that manage to call this function at the exact same time, and
   * uname() gives us the hostname for multiple machines accessing the same
   * storage.
   *
   * If someone disables posix we create a random number of the same size, so
   * this method should also work on Windows - if you manage to get maildir
   * working on Windows. Microtime could also be disabled, although I've never
   * seen it.
   *
   ...
   *
   * @throws \Zend\Mail\Storage\Exception\RuntimeException
   * @return  array array('dirname' => dir of maildir folder, 'uniq' => unique
   *   id, 'filename' => name of create file
   ...
   *
   * @throws \Zend\Mail\Storage\Exception\InvalidArgumentException
   * @return  string info string for version 2 filenames including the leading
   *   colon
   */
   ...
   * @param   null|string|\Zend\Mail\Storage\Folder $folder folder for new
   *   message, else current folder is taken
   ...
   * @param  string|\Zend\Mail\Storage\Folder $folder name or instance of
   *   targer folder
   *
   * @throws \Zend\Mail\Storage\Exception\RuntimeException
   */
   ...
   * @see _calculateQuota()
   *
   * @param bool $detailedResponse return known data of quota and current size
   *   and message count
   * @param bool $forceRecalc
   *
