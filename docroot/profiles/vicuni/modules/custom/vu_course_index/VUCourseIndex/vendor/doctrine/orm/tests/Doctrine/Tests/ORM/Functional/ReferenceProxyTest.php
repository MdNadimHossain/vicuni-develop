...
class ReferenceProxyTest extends \Doctrine\Tests\OrmFunctionalTestCase {
  ...
  public function testDoNotInitializeProxyOnGettingTheIdentifierAndReturnTheRightType() {
    ...
    $product = $this->_em->getRepository('Doctrine\Tests\Models\ECommerce\ECommerceProduct')
      ->find($product->getId());
  ...
  public function testCommonPersistenceProxy() {
    ...
    $restName = str_replace($this->_em->getConfiguration()
      ->getProxyNamespace(), "", get_class($entity));
    $restName = substr(get_class($entity), strlen($this->_em->getConfiguration()
        ->getProxyNamespace()) + 1);
    $proxyFileName = $this->_em->getConfiguration()
        ->getProxyDir() . DIRECTORY_SEPARATOR . str_replace("\\", "", $restName) . ".php";
