...
abstract class OrmFunctionalTestCase extends OrmTestCase {
  ...
  /**
   * Shared connection when a TestCase is run alone (outside of its functional
   * suite).
   *
  ...
  protected function setUp() {
    ...
    if (isset($GLOBALS['DOCTRINE_MARK_SQL_LOGS'])) {
      if (in_array(static::$_sharedConn->getDatabasePlatform()
        ->getName(), array("mysql", "postgresql"))) {
        static::$_sharedConn->executeQuery('SELECT 1 /*' . get_class($this) . '*/');
      }
      else {
        if (static::$_sharedConn->getDatabasePlatform()
            ->getName() == "oracle"
        ) {
   ...
   *
   * @param \Doctrine\ORM\Configuration $config The Configuration to pass to
   *   the EntityManager.
   * @param \Doctrine\Common\EventManager $eventManager The EventManager to
   *   pass to the EntityManager.
   *
  ...
  /**
   * Using the SQL Logger Stack this method retrieves the current query count
