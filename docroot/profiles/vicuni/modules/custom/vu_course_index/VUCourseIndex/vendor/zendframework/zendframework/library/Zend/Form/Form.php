...
class Form extends Fieldset implements FormInterface {
  ...
  /**
   * Base fieldset to use for hydrating (if none specified, directly hydrate
   * elements)
   *
  ...
  public function bind($object, $flags = FormInterface::VALUES_NORMALIZED) {
    if (!in_array($flags, array(
      FormInterface::VALUES_NORMALIZED,
      FormInterface::VALUES_RAW
    ))
    ) {
      throw new Exception\InvalidArgumentException(sprintf(
  ...
  public function setBindOnValidate($bindOnValidateFlag) {
    if (!in_array($bindOnValidateFlag, array(
  ...
  /**
   * Prepare the validation group in case Collection elements were used (this
   * function also handle the case where elements could have been dynamically
   * added or removed from a collection using JavaScript)
   *
  ...
  /**
   * Set flag indicating whether or not to scan elements and fieldsets for
   * defaults
   *
  ...
  /**
   * Set flag indicating whether or not to prefer the form input filter over
   * element and fieldset defaults
   *
  ...
  /**
   * Should we use form input filter over element input filter defaults from
   * elements and fieldsets?
   *
   * @return bool
   */
   ...
   *
   * @param  InputFilterInterface $inputFilter
   * @param  FieldsetInterface $fieldset Fieldset to traverse when looking for
   *   default inputs
   *
   * @return void
   */
      ...
      if (!$childFieldset instanceof InputFilterProviderInterface) {
        if (!$inputFilter->has($name)) {
          ...
          if ($childFieldset->getObject() instanceof InputFilterAwareInterface) {
            $inputFilter->add($childFieldset->getObject()
              ->getInputFilter(), $name);
          }
          else {
  ...
  /**
   * If true, form elements/fieldsets name's are wrapped around the form name
   * itself
   *
   * @return bool
   */
