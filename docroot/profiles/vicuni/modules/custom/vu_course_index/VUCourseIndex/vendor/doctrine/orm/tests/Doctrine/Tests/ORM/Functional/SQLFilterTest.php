...
class SQLFilterTest extends \Doctrine\Tests\OrmFunctionalTestCase {
  ...
  public function testRepositoryFind() {
    ...
    $this->assertNotNull($this->_em->getRepository('Doctrine\Tests\Models\CMS\CmsGroup')
      ->find($this->groupId));
    $this->assertNotNull($this->_em->getRepository('Doctrine\Tests\Models\CMS\CmsGroup')
      ->find($this->groupId2));
    ...
    $this->assertNotNull($this->_em->getRepository('Doctrine\Tests\Models\CMS\CmsGroup')
      ->find($this->groupId));
    $this->assertNull($this->_em->getRepository('Doctrine\Tests\Models\CMS\CmsGroup')
      ->find($this->groupId2));
  }
  ...
  public function testRepositoryFindAll() {
    ...
    $this->assertCount(2, $this->_em->getRepository('Doctrine\Tests\Models\CMS\CmsGroup')
      ->findAll());

    $this->useCMSGroupPrefixFilter();
    $this->_em->clear();

    $this->assertCount(1, $this->_em->getRepository('Doctrine\Tests\Models\CMS\CmsGroup')
      ->findAll());
  }
  ...
  public function testRepositoryFindBy() {
    ...
    $this->assertCount(1, $this->_em->getRepository('Doctrine\Tests\Models\CMS\CmsGroup')
      ->findBy(array('id' => $this->groupId2)));

    $this->useCMSGroupPrefixFilter();
    $this->_em->clear();

    $this->assertCount(0, $this->_em->getRepository('Doctrine\Tests\Models\CMS\CmsGroup')
      ->findBy(array('id' => $this->groupId2)));
  }
  ...
  public function testRepositoryFindOneBy() {
    ...
    $this->assertNotNull($this->_em->getRepository('Doctrine\Tests\Models\CMS\CmsGroup')
      ->findOneBy(array('id' => $this->groupId2)));
  ...
  public function testToOneFilter() {
    ...
    $this->_em->getFilters()
      ->enable("country")
      ->setParameter("country", "Germany", DBALType::STRING);
  ...
  public function testManyToManyFilter() {
    ...
    $this->_em->getFilters()
      ->enable("group_prefix")
      ->setParameter("prefix", "bar_%", DBALType::STRING);
  ...
  private function useCMSArticleTopicFilter() {
    ...
    $this->_em->getFilters()
      ->enable("article_topic")
      ->setParameter("topic", "Test1", DBALType::STRING);
  }
  ...
  private function useCMSGroupPrefixFilter() {
    ...
    $this->_em->getFilters()
      ->enable("group_prefix")
      ->setParameter("prefix", "foo%", DBALType::STRING);
  }
  ...
  public function testJoinSubclassPersister_FilterOnlyOnRootTableWhenFetchingSubEntity() {
    ...
    // Persister
    $this->assertEquals(2, count($this->_em->getRepository('Doctrine\Tests\Models\Company\CompanyManager')
      ->findAll()));
    // SQLWalker
    $this->assertEquals(2, count($this->_em->createQuery("SELECT cm FROM Doctrine\Tests\Models\Company\CompanyManager cm")
      ->getResult()));

    // Enable the filter
    $this->usePersonNameFilter('Guilh%');

    $managers = $this->_em->getRepository('Doctrine\Tests\Models\Company\CompanyManager')
      ->findAll();
    $this->assertEquals(1, count($managers));
    $this->assertEquals("Guilherme", $managers[0]->getName());

    $this->assertEquals(1, count($this->_em->createQuery("SELECT cm FROM Doctrine\Tests\Models\Company\CompanyManager cm")
      ->getResult()));
  }

  public function testJoinSubclassPersister_FilterOnlyOnRootTableWhenFetchingRootEntity() {
    $this->loadCompanyJoinedSubclassFixtureData();
    $this->assertEquals(3, count($this->_em->getRepository('Doctrine\Tests\Models\Company\CompanyPerson')
      ->findAll()));
    $this->assertEquals(3, count($this->_em->createQuery("SELECT cp FROM Doctrine\Tests\Models\Company\CompanyPerson cp")
      ->getResult()));

    // Enable the filter
    $this->usePersonNameFilter('Guilh%');

    $persons = $this->_em->getRepository('Doctrine\Tests\Models\Company\CompanyPerson')
      ->findAll();
    $this->assertEquals(1, count($persons));
    $this->assertEquals("Guilherme", $persons[0]->getName());

    $this->assertEquals(1, count($this->_em->createQuery("SELECT cp FROM Doctrine\Tests\Models\Company\CompanyPerson cp")
      ->getResult()));
  }
  ...
  public function testSingleTableInheritance_FilterOnlyOnRootTableWhenFetchingSubEntity() {
    ...
    // Persister
    $this->assertEquals(2, count($this->_em->getRepository('Doctrine\Tests\Models\Company\CompanyFlexUltraContract')
      ->findAll()));
    // SQLWalker
    $this->assertEquals(2, count($this->_em->createQuery("SELECT cfc FROM Doctrine\Tests\Models\Company\CompanyFlexUltraContract cfc")
      ->getResult()));
    ...
    $this->assertEquals(1, count($this->_em->getRepository('Doctrine\Tests\Models\Company\CompanyFlexUltraContract')
      ->findAll()));
    $this->assertEquals(1, count($this->_em->createQuery("SELECT cfc FROM Doctrine\Tests\Models\Company\CompanyFlexUltraContract cfc")
      ->getResult()));
  }

  public function testSingleTableInheritance_FilterOnlyOnRootTableWhenFetchingRootEntity() {
    $this->loadCompanySingleTableInheritanceFixtureData();
    $this->assertEquals(4, count($this->_em->getRepository('Doctrine\Tests\Models\Company\CompanyFlexContract')
      ->findAll()));
    $this->assertEquals(4, count($this->_em->createQuery("SELECT cfc FROM Doctrine\Tests\Models\Company\CompanyFlexContract cfc")
      ->getResult()));
    ...
    $this->assertEquals(2, count($this->_em->getRepository('Doctrine\Tests\Models\Company\CompanyFlexContract')
      ->findAll()));
    $this->assertEquals(2, count($this->_em->createQuery("SELECT cfc FROM Doctrine\Tests\Models\Company\CompanyFlexContract cfc")
      ->getResult()));
  }
