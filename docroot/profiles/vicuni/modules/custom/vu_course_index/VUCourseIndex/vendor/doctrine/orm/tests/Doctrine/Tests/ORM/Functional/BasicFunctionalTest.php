...
class BasicFunctionalTest extends \Doctrine\Tests\OrmFunctionalTestCase {
  ...
  public function testBasicUnitsOfWorkWithOneToManyAssociation() {
    ...
    $this->_em->flush();
    $this->assertFalse($this->_em->getUnitOfWork()
      ->isScheduledForDelete($user));
    ...
    $this->assertEquals(\Doctrine\ORM\UnitOfWork::STATE_NEW, $this->_em->getUnitOfWork()
      ->getEntityState($user));
    $this->assertEquals(\Doctrine\ORM\UnitOfWork::STATE_NEW, $this->_em->getUnitOfWork()
      ->getEntityState($ph));
    $this->assertEquals(\Doctrine\ORM\UnitOfWork::STATE_NEW, $this->_em->getUnitOfWork()
  ...
  public function testRemove() {
    ...
    $this->assertEquals(\Doctrine\ORM\UnitOfWork::STATE_NEW, $this->_em->getUnitOfWork()
      ->getEntityState($user), "State should be UnitOfWork::STATE_NEW");

    $this->_em->persist($user);

    $this->assertEquals(\Doctrine\ORM\UnitOfWork::STATE_MANAGED, $this->_em->getUnitOfWork()
      ->getEntityState($user), "State should be UnitOfWork::STATE_MANAGED");
    ...
    $this->assertEquals(\Doctrine\ORM\UnitOfWork::STATE_REMOVED, $this->_em->getUnitOfWork()
      ->getEntityState($user), "State should be UnitOfWork::STATE_REMOVED");
    $this->_em->flush();
  ...
  public function testOneToManyOrphanRemoval() {
    ...
    $count = $this->_em->getConnection()
      ->fetchColumn("SELECT COUNT(*) FROM cms_phonenumbers");
    $this->assertEquals(2, $count); // only 2 remaining

    // check that clear() removes the others via orphan removal
    $user->getPhonenumbers()->clear();
    $this->_em->flush();
    $this->assertEquals(0, $this->_em->getConnection()
      ->fetchColumn("select count(*) from cms_phonenumbers"));
  }
  ...
  public function testFlushDoesNotIssueUnnecessaryUpdates() {
    ...
    $oldLogger = $this->_em->getConnection()
      ->getConfiguration()
      ->getSQLLogger();
  ...
  public function testRemoveEntityByReference() {
    ...
    $this->assertEquals(0, $this->_em->getConnection()
      ->fetchColumn("select count(*) from cms_users"));
  ...
  public function testOneToOneNullUpdate() {
    ...
    $this->assertEquals(1, $this->_em->getConnection()
      ->fetchColumn("select 1 from cms_addresses where user_id = " . $user->id));

    $address->user = NULL;
    $this->_em->flush();

    $this->assertNotEquals(1, $this->_em->getConnection()
      ->fetchColumn("select 1 from cms_addresses where user_id = " . $user->id));
  }
  ...
  public function testOneToOneOrphanRemoval() {
    ...
    $this->assertEquals(0, $this->_em->getConnection()
      ->fetchColumn("select count(*) from cms_addresses"));
  ...
  public function testOneToOneMergeSetNull() {
    ...
    $this->assertNull($this->_em->find(get_class($ph), $ph->phonenumber)
      ->getUser());
  }
