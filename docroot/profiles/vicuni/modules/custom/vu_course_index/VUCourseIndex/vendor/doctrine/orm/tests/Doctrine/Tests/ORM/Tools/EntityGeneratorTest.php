...
class EntityGeneratorTest extends OrmTestCase {
  ...
  public function generateBookEntityFixture(array $embeddedClasses = array()) {
    ...
    $metadata->mapField(array(
      'fieldName' => 'status',
      'type' => 'string',
      'options' => array('default' => 'published')
    ));
    $metadata->mapField(array(
      'fieldName' => 'id',
      'type' => 'integer',
  ...
  public function testEntityUpdatingWorks() {
    ...
    $this->assertTrue($reflClass->getProperty('test')
      ->isProtected(), "Check for protected property test failed.");
    $this->assertTrue($reflClass->hasProperty('testEmbedded'), "Check for property testEmbedded failed.");
    $this->assertTrue($reflClass->getProperty('testEmbedded')
      ->isProtected(), "Check for protected property testEmbedded failed.");
    $this->assertTrue($reflClass->hasMethod('getTest'), "Check for method 'getTest' failed.");
    $this->assertTrue($reflClass->getMethod('getTest')
      ->isPublic(), "Check for public visibility of method 'getTest' failed.");
    $this->assertTrue($reflClass->hasMethod('setTest'), "Check for method 'setTest' failed.");
    $this->assertTrue($reflClass->getMethod('setTest')
      ->isPublic(), "Check for public visibility of method 'setTest' failed.");
    ...
    $metadata->mapManyToMany(array(
      ...
      'joinTable' => array(
        ...
        'joinColumns' => array(
          array(
            'name' => 'idorcamento',
            'referencedColumnName' => 'idorcamento'
          ),
