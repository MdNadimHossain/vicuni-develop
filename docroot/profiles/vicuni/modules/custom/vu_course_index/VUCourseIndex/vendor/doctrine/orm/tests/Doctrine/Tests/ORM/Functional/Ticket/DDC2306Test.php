...
class DDC2306Test extends \Doctrine\Tests\OrmFunctionalTestCase {
   ...
   *
   * The problem resides in the refresh hint passed to {@see
   * \Doctrine\ORM\UnitOfWork::createEntity}, which, as of DDC-1734, causes the
   * proxy to be marked as un-managed. The check against the identity map only
   * uses the identifier hash and the passed in class name, and does not take
   * into account the fact that the set refresh hint may be for an entity of a
   * different type from the one passed to {@see
   * \Doctrine\ORM\UnitOfWork::createEntity}
   *
   * As a result, a refresh requested for an entity `Foo` with identifier `123`
   * may cause a proxy of type `Bar` with identifier `123` to be marked as
   * un-managed.
   */
...
class DDC2306Address {
   ...
   * @var DDC2306UserAddress[]|\Doctrine\Common\Collections\Collection
   *
   * @OneToMany(targetEntity="DDC2306UserAddress", mappedBy="address",
   *   orphanRemoval=true)
   */
  public $users;
