...
class SQLParserUtilsTest extends \Doctrine\Tests\DbalTestCase {
  public function dataGetPlaceholderPositions() {
    return array(
      ...
      // Ticket DBAL-552
      array("SELECT [?] FROM foo", TRUE, array()),
      array("SELECT 'Doctrine\DBAL?' FROM foo", TRUE, array()),
      // Ticket DBAL-558
      array('SELECT "Doctrine\DBAL?" FROM foo', TRUE, array()),
      // Ticket DBAL-558
      array('SELECT `Doctrine\DBAL?` FROM foo', TRUE, array()),
      ...
      // Ticket DBAL-552
      array("SELECT [?] FROM foo WHERE bar = ?", TRUE, array(32)),
      array("SELECT 'Doctrine\DBAL?' FROM foo WHERE bar = ?", TRUE, array(45)),
      // Ticket DBAL-558
      array('SELECT "Doctrine\DBAL?" FROM foo WHERE bar = ?', TRUE, array(45)),
      // Ticket DBAL-558
      array('SELECT `Doctrine\DBAL?` FROM foo WHERE bar = ?', TRUE, array(45)),
...
SQLDATA
      ,
        FALSE,
        array(74 => 'a_param1', 91 => 'a_param2', 190 => 'a_param3')
      ),
