...
class DataAccessTest extends \Doctrine\Tests\DbalFunctionalTestCase {
  ...
  public function testFetchAllWithTypes() {
    ...
    $data = $this->_conn->fetchAll($sql, array(
      1,
      $datetime
    ), array(PDO::PARAM_STR, Type::DATETIME));
  ...
  public function testFetchBoth() {
    $sql = "SELECT test_int, test_string FROM fetch_table WHERE test_int = ? AND test_string = ?";
    $row = $this->_conn->executeQuery($sql, array(1, 'foo'))
  ...
  public function testFetchAssocWithTypes() {
    ...
    $datetime = new \DateTime($datetimeString);
    $sql = "SELECT test_int, test_datetime FROM fetch_table WHERE test_int = ? AND test_datetime = ?";
    $row = $this->_conn->fetchAssoc($sql, array(
      1,
      $datetime
    ), array(PDO::PARAM_STR, Type::DATETIME));
  ...
  public function testFetchArrayWithTypes() {
    ...
    $datetime = new \DateTime($datetimeString);
    $sql = "SELECT test_int, test_datetime FROM fetch_table WHERE test_int = ? AND test_datetime = ?";
    $row = $this->_conn->fetchArray($sql, array(
      1,
      $datetime
    ), array(PDO::PARAM_STR, Type::DATETIME));
  ...
  public function testFetchColumnWithTypes() {
    ...
    $datetime = new \DateTime($datetimeString);
    $sql = "SELECT test_int, test_datetime FROM fetch_table WHERE test_int = ? AND test_datetime = ?";
    $column = $this->_conn->fetchColumn($sql, array(
      1,
      $datetime
    ), 1, array(PDO::PARAM_STR, Type::DATETIME));
  ...
  public function testTrimExpression($value, $position, $char, $expectedResult) {
    $sql = 'SELECT ' .
      $this->_conn->getDatabasePlatform()
        ->getTrimExpression($value, $position, $char) . ' AS trimmed ' .
      'FROM fetch_table';
  ...
  public function testEmptyFetchColumnReturnsFalse() {
    ...
    $this->assertFalse($this->_conn->query('SELECT test_int FROM fetch_table')
      ->fetchColumn());
  }
