...
class Part implements RecursiveIterator, Part\PartInterface {
   ...
   * - headers    headers as array (name => value) or string, if a content part
   * is found it's used as toplines
  ...
  protected function _cacheContent() {
    ...
    foreach ($parts as $part) {
      $this->parts[$counter++] = new static(array(
        'headers' => $part['header'],
        'content' => $part['body']
      ));
    }
   ...
   *
   * Internally headers that occur more than once are saved as array, all other
   * as string. If $format is set to string implode is used to concat the
   * values (with Mime::LINEEND as delim).
   *
  ...
  /**
   * Get a specific field from a header like content type or all fields as
   * array
   *
   ...
   *
   * @return string|array wanted part or all parts as array($firstName =>
   *   firstPart, partname => value)
