...
class Client implements Stdlib\DispatchableInterface {
  ...
  public function setUri($uri) {
    if (!empty($uri)) {
      ...
      if ($this->getUri()->getUser() && $this->getUri()->getPassword()) {
        $this->setAuth($this->getUri()->getUser(), $this->getUri()
          ->getPassword());
      }

      // We have no ports, set the defaults
      if (!$this->getUri()->getPort()) {
        $this->getUri()->setPort(($this->getUri()
   ...
   *
   * @param  bool $clearCookies Also clear all valid cookies? (defaults to
   *   false)
   ...
   *
   * @param string|bool $streamfile Stream file, true for temp file, false/null
   *   for no streaming
   *
    ...
    switch (strtolower($type)) {
      ...
      case self::AUTH_DIGEST :
        ...
        elseif (strtolower($digest['qop']) == 'auth-int') {
          ...
          $ha2 = md5($this->getMethod() . ':' . $this->getUri()
              ->getPath() . ':' . md5($entityBody));
        }
        if (empty($digest['qop'])) {
  ...
  public function send(Request $request = NULL) {
    ...
    do {
      ...
      if ($response->isRedirect() && ($response->getHeaders()
          ->has('Location'))
      ) {

        // Avoid problems with buggy servers that add whitespace at the
        // end of some headers
        $location = trim($response->getHeaders()
          ->get('Location')
          ->getFieldValue());
   ...
   *
   * 1. $data is null (default): $filename is treated as the name if a local
   * file which will be read and sent. Will try to guess the content type using
   * mime_content_type().
   * 2. $data is set - $filename is sent as the file name, but $data is sent as
   * the file contents and no file is read from the file system. In this case,
   * you need to manually set the Content-Type ($ctype) or it will default to
  ...
  protected function prepareHeaders($body, $uri) {
    ...
    if (!$this->getRequest()
        ->getHeaders()
        ->has('User-Agent') && isset($this->config['useragent'])
    ) {
  ...
  protected function prepareBody() {
    ...
    if (count($this->getRequest()
        ->getPost()
        ->toArray()) > 0 || $totalFiles > 0
    ) {
      if (stripos($this->getEncType(), self::ENC_FORMDATA) === 0) {
        ...
        $params = self::flattenParametersArray($this->getRequest()
          ->getPost()
          ->toArray());
   ...
   * @param mixed $value
   * @param string $filename
   * @param array $headers Associative array of optional headers @example
