...
class MemcachedResourceManager {
   ...
   * @return array array('host' => <host>, 'port' => <port>, 'weight' =>
   *   <weight>)
   */
  ...
  public function getResource($id) {
    ...
    $servers = array_udiff($resource['servers'], $memc->getServerList(), array(
      $this,
      'compareServers'
    ));
    if ($servers) {
  ...
  protected function normalizeLibOptionKey(& $key) {
    // convert option name into it's constant value
    if (is_string($key)) {
      $const = 'Memcached::OPT_' . str_replace(array(
          ' ',
          '-'
        ), '_', strtoupper($key));
      if (!defined($const)) {
   ...
   * - Assoc: array('host' => <host>[, 'port' => <port>][, 'weight' =>
   * <weight>])
  ...
  public function setServers($id, $servers) {
    ...
    if ($resource instanceof MemcachedResource) {
      ...
      $servers = array_udiff($servers, $resource->getServerList(), array(
        $this,
        'compareServers'
      ));
      if ($servers) {
  ...
  public function addServers($id, $servers) {
    ...
    if ($resource instanceof MemcachedResource) {
      ...
      $resource['servers'] = array_merge(
        $resource['servers'],
        array_udiff($servers, $resource['servers'], array(
          $this,
          'compareServers'
        ))
      );
