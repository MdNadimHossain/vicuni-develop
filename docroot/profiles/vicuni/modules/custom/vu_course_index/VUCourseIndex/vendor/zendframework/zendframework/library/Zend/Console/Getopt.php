 ...
 * Flag: the letter or word set off by a '-' or '--'.  Example: in '--output
 * filename',
 *   '--output' is the flag.
...
class Getopt {
   ...
   * parseAll is true if all options on the command line should be parsed,
   * regardless of whether an argument appears before them.
   */
   ...
   * @param  array $argv
   *
   * @throws Exception\InvalidArgumentException When not given an array as
   *   parameter
   * @return self
   */
  ...
  /**
   * @param string $option The name of the property which, if present, will
   *   call the passed callback with the value of this parameter.
   * @param callable $callback The callback that will be called for this
   *   option. The first parameter will be the value of getOption($option), the
   *   second parameter will be a reference to $this object. If the callback
   *   returns false then an Exception\RuntimeException will be thrown
   *   indicating that there is a parse issue with this option.
   *
   * @return self
   */
