...
class Pop3 {
   ...
   * Public constructor
   *
   * @param  string $host hostname or IP address of POP3 server, if given
   *   connect() is called
   * @param  int|null $port port of POP3 server, null for default (110 or 995
   *   for ssl)
   ...
   * read a response
   *
   * @param  bool $multiline response has multiple lines and should be read
   *   until "<nl>.<nl>"
   *
   * @throws Exception\RuntimeException
   ...
   * @param  int|null $msgno number of message, null for all
   *
   * @return string|array uniqueid of message or list with array(num =>
   *   uniqueid)
   */
   ...
   * This method also sets hasTop - before it it's not known if top is
   * supported
   *
   * The fallback makes normal RETR call, which retrieves the whole message.
   * Additional lines are not removed.
   *
   * @param  int $msgno number of message
   * @param  int $lines number of wanted body lines (empty line is inserted
   *   after header lines)
