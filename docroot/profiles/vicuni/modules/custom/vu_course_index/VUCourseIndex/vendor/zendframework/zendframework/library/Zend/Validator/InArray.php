...
class InArray extends AbstractValidator {
  ...
  /**
   * Non strict check but prevents "asdf" == 0 returning TRUE causing
   * false/positive. This is the most secure option for non-strict checks and
   * replaces strict = false This will only be effective when the input is a
   * string
   */
  ...
  /**
   * Type of strict check to be used. Due to "foo" == 0 === TRUE with in_array
   * when strict = false, an option has been added to prevent this. When
   * $strict = 0/false, the most secure non-strict check is implemented. if
   * $strict = -1, the default in_array non-strict behaviour is used
   *
   * @var int
   */
   ...
   * InArray::CHECK_STRICT |
   * InArray::CHECK_NOT_STRICT_AND_PREVENT_STR_TO_INT_VULNERABILITY |
  ...
  /**
   * Returns true if and only if $value is contained in the haystack option. If
   * the strict option is true, then the type of $value is also checked.
   *
   * @param mixed $value
