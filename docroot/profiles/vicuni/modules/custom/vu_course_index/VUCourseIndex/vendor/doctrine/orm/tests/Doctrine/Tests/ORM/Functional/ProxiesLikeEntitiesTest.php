...
/**
 * Test that Doctrine ORM correctly works with proxy instances exactly like
 * with ordinary Entities
 *
...
class ProxiesLikeEntitiesTest extends \Doctrine\Tests\OrmFunctionalTestCase {
  ...
  public function testPersistUpdate() {
    // Considering case (a)
    $proxy = $this->_em->getProxyFactory()
      ->getProxy('Doctrine\Tests\Models\CMS\CmsUser', array('id' => 123));
    ...
    $this->assertNotEmpty($this->_em->getUnitOfWork()
      ->getEntityChangeSet($proxy));
  ...
  public function testProxyAsDqlParameterPersist() {
    $proxy = $this->_em->getProxyFactory()
      ->getProxy('Doctrine\Tests\Models\CMS\CmsUser', array('id' => $this->user->getId()));
  ...
  protected function tearDown() {
    $this->_em->createQuery('DELETE FROM Doctrine\Tests\Models\CMS\CmsUser u')
      ->execute();
  }
}
