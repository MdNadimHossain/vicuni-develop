...
class PostgreSqlSchemaManagerTest extends SchemaManagerFunctionalTestCase {
  ...
  public function testSupportDomainTypeFallback() {
    ...
    $table = $this->_conn->getSchemaManager()
      ->listTableDetails('domain_type_test');
    $this->assertInstanceOf('Doctrine\DBAL\Types\DecimalType', $table->getColumn('value')
      ->getType());

    Type::addType('MyMoney', 'Doctrine\Tests\DBAL\Functional\Schema\MoneyType');
    $this->_conn->getDatabasePlatform()
      ->registerDoctrineTypeMapping('MyMoney', 'MyMoney');

    $table = $this->_conn->getSchemaManager()
      ->listTableDetails('domain_type_test');
    $this->assertInstanceOf('Doctrine\Tests\DBAL\Functional\Schema\MoneyType', $table->getColumn('value')
      ->getType());
  }
  ...
  public function testAlterTableAutoIncrementDrop() {
    ...
    $this->assertEquals(array("ALTER TABLE autoinc_table_drop ALTER id DROP DEFAULT"), $this->_conn->getDatabasePlatform()
  ...
  public function testTableWithSchema() {
    ...
    $this->assertEquals(array('id'), $nestedSchemaTable->getPrimaryKey()
      ->getColumns());
  ...
  public function testFilterSchemaExpression() {
    ...
    $this->_conn->getConfiguration()
      ->setFilterSchemaAssetsExpression('#^dbal204_#');
    $names = $this->_sm->listTableNames();
    $this->assertEquals(2, count($names));

    $this->_conn->getConfiguration()
      ->setFilterSchemaAssetsExpression('#^dbal204_test#');
    $names = $this->_sm->listTableNames();
  ...
  public function testListTableWithBinary() {
    ...
    $this->assertInstanceOf('Doctrine\DBAL\Types\BlobType', $table->getColumn('column_varbinary')
      ->getType());
    $this->assertFalse($table->getColumn('column_varbinary')->getFixed());

    $this->assertInstanceOf('Doctrine\DBAL\Types\BlobType', $table->getColumn('column_binary')
      ->getType());
  ...
  public function testPartialIndexes() {
    ...
    $offlineTable->addUniqueIndex(array(
      'id',
      'name'
    ), 'simple_partial_index', array('where' => '(id IS NULL)'));
    $offlineTable->addIndex(array(
      'id',
      'name'
    ), 'complex_partial_index', array(), array('where' => '(((id IS NOT NULL) AND (name IS NULL)) AND (email IS NULL))'));
    ...
    $this->assertTrue($onlineTable->getIndex('simple_partial_index')
      ->hasOption('where'));
    $this->assertTrue($onlineTable->getIndex('complex_partial_index')
      ->hasOption('where'));
    $this->assertSame('(id IS NULL)', $onlineTable->getIndex('simple_partial_index')
      ->getOption('where'));
    $this->assertSame(
