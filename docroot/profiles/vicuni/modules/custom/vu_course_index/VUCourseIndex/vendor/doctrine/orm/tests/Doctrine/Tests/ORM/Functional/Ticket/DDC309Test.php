...
class DDC309Test extends \Doctrine\Tests\OrmFunctionalTestCase {
  ...
  public function testTwoIterateHydrations() {
    ...
    $q = $this->_em->createQuery('SELECT c FROM Doctrine\Tests\ORM\Functional\Ticket\DDC309Country c')
      ->iterate();
    $c = $q->next();

    $this->assertEquals(1, $c[0]->id);

    $r = $this->_em->createQuery('SELECT u FROM Doctrine\Tests\ORM\Functional\Ticket\DDC309User u')
      ->iterate();
