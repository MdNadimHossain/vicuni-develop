...
class PortabilityTest extends \Doctrine\Tests\DbalFunctionalTestCase {
  ...
  private function getPortableConnection($portabilityMode = \Doctrine\DBAL\Portability\Connection::PORTABILITY_ALL, $case = \PDO::CASE_LOWER) {
    if (!$this->portableConnection) {
      ...
      try {
        ...
        $table->addColumn('Test_String', 'string', array(
          'fixed' => TRUE,
          'length' => 32
        ));
        ...
        $this->portableConnection->insert('portability_table', array(
          'Test_Int' => 1,
          'Test_String' => 'foo',
          'Test_Null' => ''
        ));
  ...
  public function testFullFetchMode() {
    $rows = $this->getPortableConnection()
      ->fetchAll('SELECT * FROM portability_table');
    $this->assertFetchResultRows($rows);

    $stmt = $this->getPortableConnection()
      ->query('SELECT * FROM portability_table');
  ...
  public function assertFetchResultRow($row) {
    $this->assertTrue(in_array($row['test_int'], array(
      1,
      2
