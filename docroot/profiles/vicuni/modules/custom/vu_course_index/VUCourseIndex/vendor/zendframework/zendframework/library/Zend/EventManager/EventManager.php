...
class EventManager implements EventManagerInterface {
  ...
  /**
   * Identifiers, used to pull shared signals from SharedEventManagerInterface
   * instance
   *
   * @var array
   */
   ...
   *
   * @param  string $event
   * @param  string|object $target Object calling emit, or symbol describing
   *   target (such as static method name)
   ...
   *
   * @param  string|array|ListenerAggregateInterface $event An event or array
   *   of event names. If a ListenerAggregateInterface, proxies to {@link
   *   attachAggregate()}.
   * @param  callable|int $callback If string $event provided, expects PHP
   *   callback; for a ListenerAggregateInterface $event, this will be the
   *   priority
   * @param  int $priority If provided, the priority at which to register the
   *   callable
   *
   * @return CallbackHandler|mixed CallbackHandler if attaching callable (to
   *   allow later unsubscribe); mixed if attaching aggregate
  ...
  public function attach($event, $callback = NULL, $priority = 1) {
    ...
    $listener = new CallbackHandler($callback, array(
      'event' => $event,
      'priority' => $priority
    ));
   ...
   *
   * Listener aggregates accept an EventManagerInterface instance, and call
   * attach() one or more times, typically to attach to multiple events using
   * local methods.
   *
   * @param  ListenerAggregateInterface $aggregate
   * @param  int $priority If provided, a suggested priority for the aggregate
   *   to use
   *
   ...
   *
   * @return bool Returns true if event and listener found, and unsubscribed;
   *   returns false if either event or listener not found
   ...
   *
   * Listener aggregates accept an EventManagerInterface instance, and call
   * detach() of all previously attached listeners.
   *
   * @param  ListenerAggregateInterface $aggregate
   *
   ...
   * Trigger listeners
   *
   * Actual functionality for triggering listeners, to which both trigger() and
   * triggerUntil() delegate.
   *
