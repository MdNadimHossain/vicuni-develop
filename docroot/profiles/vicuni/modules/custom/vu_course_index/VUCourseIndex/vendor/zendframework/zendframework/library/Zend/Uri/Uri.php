...
class Uri implements UriInterface {
  ...
  const CHAR_RESERVED = ':\/\?#\[\]@!\$&\'\(\)\*\+,;=';
  /**
   * Not in the spec - those characters have special meaning in urlencoded
   * query parameters
   */
  ...
  /**
   * Check if a host name is a valid IP address, depending on allowed IP
   * address types
   *
   * @param  string $host
  ...
  /**
   * Check if an address is a valid registered name (as defined by RFC-3986)
   * address
   *
   * @param  string $host
   *
  ...
  /**
   * Decode all percent encoded characters which are allowed to be represented
   * literally
   *
