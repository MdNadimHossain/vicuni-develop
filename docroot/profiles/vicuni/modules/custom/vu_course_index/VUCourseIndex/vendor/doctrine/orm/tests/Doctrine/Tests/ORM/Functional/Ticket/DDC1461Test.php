...
class DDC1461Test extends \Doctrine\Tests\OrmFunctionalTestCase {
  ...
  public function testChangeDetectionDeferredExplicit() {
    ...
    $this->assertEquals(\Doctrine\ORM\UnitOfWork::STATE_MANAGED, $this->_em->getUnitOfWork()
      ->getEntityState($user, \Doctrine\ORM\UnitOfWork::STATE_NEW), "Entity should be managed.");
    $this->assertEquals(\Doctrine\ORM\UnitOfWork::STATE_MANAGED, $this->_em->getUnitOfWork()
      ->getEntityState($user), "Entity should be managed.");
...
class DDC1461User {
  ...
  /**
   * @OneToOne(targetEntity="DDC1461TwitterAccount", orphanRemoval=true,
   *   fetch="EAGER", cascade = {"persist"}, inversedBy="user")
   * @var TwitterAccount
   */
