 ...
 *   a.country, a.zip, a.city FROM cms_users u INNER JOIN cms_addresses a ON
 *   u.id = a.user_id WHERE u.username = ?"
 *      ),
 *      @NamedNativeQuery(
 *          name            = "fetchJoinedPhonenumber",
 *          resultSetMapping= "mappingJoinedPhonenumber",
 *          query           = "SELECT id, name, status, phonenumber AS number
 *   FROM cms_users INNER JOIN cms_phonenumbers ON id = user_id WHERE username
 *   = ?"
 *      ),
 *      @NamedNativeQuery(
 *          name            = "fetchUserPhonenumberCount",
 *          resultSetMapping= "mappingUserPhonenumberCount",
 *          query           = "SELECT id, name, status, COUNT(phonenumber) AS
 *   numphones FROM cms_users INNER JOIN cms_phonenumbers ON id = user_id WHERE
 *   username IN (?) GROUP BY id, name, status, username ORDER BY username"
 *      ),
 *      @NamedNativeQuery(
 ...
 *   COUNT(p.phonenumber) AS numphones FROM cms_users u INNER JOIN
 *   cms_addresses a ON u.id = a.user_id INNER JOIN cms_phonenumbers p ON u.id
 *   = p.user_id GROUP BY u.id, u.name, u.status, u.username, a.id, a.zip,
 *   a.country ORDER BY u.username"
 *      ),
 * })
 *
 ...
 *                      @FieldResult("name"),
 *                      @FieldResult("status"),
 *                      @FieldResult("phonenumbers.phonenumber" , column =
 *   "number"),
 *                  }
 *              )
 *          }
...
class CmsUser {
  ...
  public $name;
  /**
   * @OneToMany(targetEntity="CmsPhonenumber", mappedBy="user",
  ...
  public $address;
  /**
   * @OneToOne(targetEntity="CmsEmail", inversedBy="user", cascade={"persist"},
   *   orphanRemoval=true)
   * @JoinColumn(referencedColumnName="id", nullable=true)
   */
  public $email;
  /**
   * @ManyToMany(targetEntity="CmsGroup", inversedBy="users",
