...
class Controller {
  ...
  private function createOrRetrieveClassId($classType) {
    if (FALSE !== $id = $this->connection->executeQuery($this->getSelectClassIdSql($classType))
        ->fetchColumn()
    ) {
      return $id;
    }
    ...
    return $this->connection->executeQuery($this->getSelectClassIdSql($classType))
      ->fetchColumn();
  }
  ...
  private function createOrRetrieveSecurityIdentityId(SecurityIdentityInterface $sid) {
    if (FALSE !== $id = $this->connection->executeQuery($this->getSelectSecurityIdentityIdSql($sid))
        ->fetchColumn()
    ) {
      return $id;
    }
    ...
    return $this->connection->executeQuery($this->getSelectSecurityIdentityIdSql($sid))
      ->fetchColumn();
  }
  ...
  /**
   * This processes changes on an ACE related property (classFieldAces, or
   * objectFieldAces).
   *
   * @param string $name
   * @param array $changes
    ...
    foreach ($changes[1] as $field => $new) {
      for ($i = 0, $c = count($new); $i < $c; $i++) {
        ...
        if (NULL === $ace->getId()) {
          ...
          $objectIdentityId = $name === 'classFieldAces' ? NULL : $ace->getAcl()
            ->getId();
          ...
          $aceId = $this->connection->executeQuery($this->getSelectAccessControlEntryIdSql($classId, $objectIdentityId, $field, $i))
            ->fetchColumn();
  ...
  /**
   * This processes changes on an ACE related property (classAces, or
   * objectAces).
   *
   * @param string $name
   * @param array $changes
  ...
  private function updateAceProperty($name, array $changes) {
    ...
    for ($i = 0, $c = count($new); $i < $c; $i++) {
      ...
      if (NULL === $ace->getId()) {
        ...
        $objectIdentityId = $name === 'classAces' ? NULL : $ace->getAcl()
          ->getId();
        ...
        $aceId = $this->connection->executeQuery($this->getSelectAccessControlEntryIdSql($classId, $objectIdentityId, NULL, $i))
          ->fetchColumn();
        $this->loadedAces[$aceId] = $ace;
  ...
  private function updateAces(\SplObjectStorage $aces) {
    foreach ($aces as $ace) {
      ...
      if (isset($propertyChanges['auditSuccess'])) {
        $sets[] = sprintf('audit_success = %s', $this->connection->getDatabasePlatform()
          ->convertBooleans($propertyChanges['auditSuccess'][1]));
      }
      if (isset($propertyChanges['auditFailure'])) {
        $sets[] = sprintf('audit_failure = %s', $this->connection->getDatabasePlatform()
