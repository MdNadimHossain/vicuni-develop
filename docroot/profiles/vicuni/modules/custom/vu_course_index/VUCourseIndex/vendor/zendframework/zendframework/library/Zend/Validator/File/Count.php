...
class Count extends AbstractValidator {
  ...
  protected $options = array(
    'min' => NULL,
    // Minimum file count, if null there is no minimum file count
    'max' => NULL,
    // Maximum file count, if null there is no maximum file count
  );

  /**
   * Sets validator options
   *
   * Min limits the file count, when used with max=null it is the maximum file
   * count It also accepts an array with the keys 'min' and 'max'
   *
  ...
  /**
   * Returns true if and only if the file count of all checked files is at
   * least min and not bigger than max (when max is not null). Attention: When
   * checking with set min you must give all files with the first call,
