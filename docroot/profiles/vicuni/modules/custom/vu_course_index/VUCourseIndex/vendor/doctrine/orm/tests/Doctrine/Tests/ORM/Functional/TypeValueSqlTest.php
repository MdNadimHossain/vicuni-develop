...
class TypeValueSqlTest extends \Doctrine\Tests\OrmFunctionalTestCase {
  ...
  public function testUpperCaseStringType() {
    ...
    $this->assertEquals('FOO', $this->_em->getConnection()
      ->fetchColumn("select lowerCaseString from customtype_uppercases where id=" . $entity->id . ""), 'Database holds uppercase string');
  }
  ...
  public function testUpperCaseStringTypeWhenColumnNameIsDefined() {
    ...
    $this->assertEquals('FOO', $this->_em->getConnection()
      ->fetchColumn("select named_lower_case_string from customtype_uppercases where id=" . $entity->id . ""), 'Database holds uppercase string');
    ...
    $this->assertEquals('BAR', $this->_em->getConnection()
      ->fetchColumn("select named_lower_case_string from customtype_uppercases where id=" . $entity->id . ""), 'Database holds uppercase string');
  }
  ...
  public function testTypeValueSqlWithAssociations() {
    ...
    $this->assertEquals(1, $this->_em->getConnection()
      ->fetchColumn("select customInteger from customtype_parents where id=" . $entity->id . ""), 'Database has stored customInteger positive');
