...
class DefaultCollectionHydratorTest extends OrmFunctionalTestCase {
  ...
  protected function setUp() {
    ...
    $targetPersister = $this->_em->getUnitOfWork()
      ->getEntityPersister(City::CLASSNAME);
  ...
  public function testLoadCacheCollection() {
    $targetRegion = $this->_em->getCache()
      ->getEntityCacheRegion(City::CLASSNAME);
    ...
    $targetRegion->put(new EntityCacheKey(City::CLASSNAME, array('id' => 31)), new EntityCacheEntry(City::CLASSNAME, array(
      'id' => 31,
      'name' => 'Foo'
    )));
    $targetRegion->put(new EntityCacheKey(City::CLASSNAME, array('id' => 32)), new EntityCacheEntry(City::CLASSNAME, array(
      'id' => 32,
      'name' => 'Bar'
    )));
    ...
    $this->assertEquals(UnitOfWork::STATE_MANAGED, $this->_em->getUnitOfWork()
      ->getEntityState($collection[0]));
    $this->assertEquals(UnitOfWork::STATE_MANAGED, $this->_em->getUnitOfWork()
      ->getEntityState($collection[1]));
  }
