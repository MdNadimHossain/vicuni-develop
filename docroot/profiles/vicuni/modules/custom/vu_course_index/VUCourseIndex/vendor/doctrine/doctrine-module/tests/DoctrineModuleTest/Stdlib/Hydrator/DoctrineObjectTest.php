...
class DoctrineObjectTest extends BaseTestCase {
  ...
  /**
   * When using hydration by value, it will use the public API of the entity to
   * set values (setters)
   *
  ...
  /**
   * When using hydration by reference, it won't use the public API of the
   * entity to set values (getters)
   *
  ...
  public function testHydrateOneToOneAssociationByValue() {
    ...
    $this->assertEquals('Modified from setToOne setter', $entity->getToOne(FALSE)
      ->getField(FALSE));
  }
  ...
  public function testAssertCollectionsAreNotSwappedDuringHydrationUsingIdentifiersForRelations() {
    ...
    // Set the initial collection
    $entity->addEntities(new ArrayCollection(array(
  ...
  public function testUsesStrategyOnSimpleFieldsWhenExtractingByValue() {
    ...
    $this->assertEquals(array(
      'id' => 2,
      'field' => 'modified while extracting'
    ), $data);
  }
