 ...
 * with Drupal.behaviors.responsiveMenu.attach() as a centralised plugin
...
(function ($, Drupal) {
    ...
    attach: function (context) {
      ...
      var $nav = Drupal.vuResponsiveMenu.init({
        ...
        linksSelectorsDropdownTrigger: [
          '#block-menu-block-main-menu-level1 .menu li .js-menu-item-login'
        ],
      ...
      Drupal.vuOffCanvas.init({
        $container: $menuContainer,
        $triggerOpen: $(Drupal.theme('offcanvasTrigger', 'open', 'Menu')).insertAfter($menuTriggerAnchor, context),
        $triggerClose: $(Drupal.theme('offcanvasTrigger', 'close', 'Close')).appendTo($nav),
  ...
  /**
   * Plugin to generate responsive (mobile) menu.
   */
  Drupal.vuResponsiveMenu = {
    ...
    init: function (config) {
      ...
      $secondary.find(self.selectorToQuery(self.SELECTOR_LEAF, '', '>a')).each(function () {
        ...
        $link.wrap(Drupal.theme('responsiveMenuLinkWrapper', $link));

        if (self.isLeafExpanded($leaf)) {
          // Inject current link as a parent for a child menu wrapper.
          self.getChildMenuWrapper($link).prepend(Drupal.theme('responsiveMenuParentLink', $link));

          // Inject parent link into child menu as a back button.
          var $parentLink = self.getParentLink($link);
          $parentLink = Drupal.theme('responsiveMenuBackLink', $parentLink);
          ...
          // Add forward link.
          var $forwardLink = $(Drupal.theme('responsiveMenuForwardLink'));
      ...
      // Render nav.
      var $nav = $(Drupal.theme('responsiveMenuNav', $primary)).appendTo(config.$container);
      ...
      if ($(self.config.linksSelectorsDropdownTrigger).length > 0 && $(self.config.linksSelectorsDropdownItems).length > 0) {
        ...
        if (linksDropdownItems.length > 0) {
          // Render dropdown.
          $nav.prepend($(Drupal.theme('responsiveMenuDropdown', $(linkDropdownTrigger).get(0), self.linksToMenu(linksDropdownItems, self.SELECTOR_ACTIVE_TRAIL))));
        }
      }
    ...
    extractLinks: function (linksSelectors, filterClasses) {
      ...
      var self = this;
      var links = [];
      for (var i in linksSelectors) {
        if (linksSelectors.hasOwnProperty(i)) {
  ...
  Drupal.vuFixedBlock = {
    ...
    fixedBlockAnimationCallback: function () {
      ...
      Drupal.vuFixedBlock.lastScroll = scrollTop;
  ...
  /**
   * Theme implementation for responsive menu navigation element.
   *
   * @param {string} ul
   *   HTML list with navigation items to insert.
   *
   * @returns {string}
   *   The markup for responsive menu navigation.
   */
  Drupal.theme.prototype.responsiveMenuNav = function (ul) {
    return $('<nav id="responsive-nav" class="navbar-collapse"></nav>').append($(ul));
  };

  /**
   * Theme implementation for responsive menu parent link.
   *
   * @param {object} $link
   *   Link object to use for parent menu link rendering.
   *
   * @returns {string}
   *   The markup for the responsive menu parent link.
   */
  Drupal.theme.prototype.responsiveMenuParentLink = function ($link) {
    var $newLink = $('<a></a>').addClass($link.attr('class')).attr('href', $link.attr('href')).html($link.html());
    return $('<div class="' + Drupal.vuResponsiveMenu.SELECTOR_LINK_PARENT_WRAPPER.substr(1) + '"></div>').append($newLink);
  };

  /**
   * Theme implementation for responsive menu back link.
   *
   * @param {object} $link
   *   Link object to use for back menu link rendering.
   *
   * @returns {string}
   *   The markup for the responsive menu back link.
   */
  Drupal.theme.prototype.responsiveMenuBackLink = function ($link) {
    var text = $link.length > 0 ? $link.html() : '';
    return $('<span class="' + Drupal.vuResponsiveMenu.SELECTOR_LINK_BACK.substr(1) + '">' + text + '</span>');
  };

  /**
   * Theme implementation for responsive menu forward link.
   *
   * @returns {string}
   *   The markup for the responsive menu forward link.
   */
  Drupal.theme.prototype.responsiveMenuForwardLink = function () {
    return '<span class="' + Drupal.vuResponsiveMenu.SELECTOR_LINK_FORWARD.substr(1) + '"></span>';
  };

  /**
   * Theme implementation for responsive menu link wrapper.
   *
   * @param {object} $link
   *   Link object to use for menu link wrapper rendering.
   *
   * @returns {string}
   *   The markup for the responsive menu link wrapper.
   */
  Drupal.theme.prototype.responsiveMenuLinkWrapper = function ($link) {
    return '<div class="' + Drupal.vuResponsiveMenu.SELECTOR_LINK_WRAPPER.substr(1) + '"></div>';
  };
   ...
   */
  Drupal.theme.prototype.responsiveMenuDropdown = function (trigger, itemsList) {
   ...
   */
  Drupal.theme.prototype.offcanvasTrigger = function (type, title) {
    title = title || '';
    return '<button type="button" class="js-offcanvas-trigger js-offcanvas-trigger-' + type + '" aria-controls="#responsive-nav" aria-expanded="false">' +
      '<span class="icon-bar"></span><span class="icon-bar"></span><span class="icon-bar"></span><span class="title">' + title + '</span>' +
      '</button>';
  };
